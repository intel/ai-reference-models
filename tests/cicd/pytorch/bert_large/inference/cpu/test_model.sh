...
PRECISION=$1
OUTPUT_DIR=${OUTPUT_DIR-"$(pwd)/tests/cicd/pytorch/bert_large/inference/cpu/output/${PRECISION}"}
is_lkg_drop=$2
TEST_MODE=$3
...
EVAL_DATA_FILE=$(pwd)/dev-v1.1.json
...
# Run script
OUTPUT_DIR=${OUTPUT_DIR} PRECISION=${PRECISION} FINETUNED_MODEL=${FINETUNED_MODEL} EVAL_DATA_FILE=${EVAL_DATA_FILE} TEST_MODE=${TEST_MODE} ./run_model.sh
cd -