...
def test_platform_util_lscpu_parsing(
    get_cpuset_mock, platform_mock, subprocess_mock, os_mock
):
    """
...
def test_cpu_info_binding_information(subprocess_mock):
    ...
    with open(
        os.path.join(tests_data_dir, "utils", "files", "sorted_membind_info.json")
    ) as json_data:
...
def test_cpu_info_binding_information_no_numa(subprocess_mock):
    ...
    with open(
        os.path.join(
            tests_data_dir, "utils", "files", "sorted_membind_info_no_numa.json"
        )
    ) as json_data:
        expected_value = json.load(json_data)
...
@patch("benchmarks.common.platform_util.PlatformUtil._get_cpuset")
def test_numa_cpu_core_list(
    get_cpuset_mock, subprocess_mock, subprocess_popen_mock, platform_mock, os_mock
):
    """Test the platform utils to ensure that we are getting the proper core lists"""
    subprocess_mock.return_value = platform_config.LSCPU_OUTPUT
    subprocess_popen_mock.return_value.stdout.readlines.return_value = (
...
@pytest.mark.parametrize(
    "cpuset_range,expected_core_list",
    [
        [
            "0-7,28-35",
            [
        ...
        [
            "0,2-5,20,29-32,1",
            [["0", "1", "2", "3", "4", "5", "20"], ["29", "30", "31", "32"]],
        ],
    ],
)
@patch("os.path.exists")
def test_numa_cpu_core_list_cpuset(
    path_exists_mock,
    subprocess_mock,
...
def test_platform_utils_num_sockets_with_cpuset(
    get_cpuset_mock,
    platform_mock,
    subprocess_mock,
    os_mock,
