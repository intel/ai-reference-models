...
flags.DEFINE_string(
    "dataset_option",
    "Training",
    "Option to pre-process entire dataset or just the validation dataset.",
)
...
def _process_image_files_batch(coder, output_file, filenames, synsets, labels):
    ...
    for filename, synset in zip(filenames, synsets):
        ...
        example = _convert_to_example(
            filename, image_buffer, label, synset, height, width
        )
...
def _process_dataset(filenames, synsets, labels, output_directory, prefix, num_shards):
    ...
    for shard in range(num_shards):
        ...
        output_file = os.path.join(output_directory, single_filename)
        _process_image_files_batch(
            coder, output_file, chunk_files, chunk_synsets, labels
        )
...
def convert_to_tf_records(raw_data_dir):
    ...
    validation_synsets = (
        tf.gfile.FastGFile(os.path.join(raw_data_dir, LABELS_FILE), "r")
        .read()
        .splitlines()
    )
...
def convert_val_data_to_tf_records(raw_data_dir):
    ...
    # Create unique ids for all synsets
    labels = {v: k + 1 for k, v in enumerate(sorted(set(validation_synsets)))}
