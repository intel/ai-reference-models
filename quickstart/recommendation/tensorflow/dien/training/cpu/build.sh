...
DOCKERFILE=tf-spr-dien-training.Dockerfile
TF_SPR_BASE_IMAGE=${TF_SPR_BASE_IMAGE:-model-zoo}
TF_SPR_BASE_TAG=${TF_SPR_BASE_TAG:-tensorflow-spr}
IMAGE_NAME=${IMAGE_NAME:-model-zoo:tf-spr-dien-training}
...
if [ "$(docker images -q ${TF_SPR_BASE_IMAGE}:${TF_SPR_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building DIEN training container."
