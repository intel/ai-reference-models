...
if [ ! -e "${MODEL_DIR}/models/recommendation/pytorch/memrec_dlrm/product/memrec.py"  ]; then
    ...
    echo "From which the memrec.py exist at the: \${MODEL_DIR}/models/recommendation/pytorch/memrec_dlrm/product/memrec.py"
    exit 1
fi
...
mkdir -p ${OUTPUT_DIR}/memrec_inference_accuracy_log

LOG=${OUTPUT_DIR}/memrec_inference_accuracy_log/${PRECISION}
...
--print-freq=1000 --print-time --mini-batch-size=2048 --test-mini-batch-size=16384 \
...
echo ""memrec";"auc";${PRECISION};16384;${accuracy}" | tee -a ${OUTPUT_DIR}/summary.log
