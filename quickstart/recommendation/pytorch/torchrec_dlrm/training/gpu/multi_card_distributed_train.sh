...
TOTAL_TRAINING_SAMPLES=${TOTAL_TRAINING_SAMPLES-4195197692}
...
echo "Running DLRM single-node distributed multi-card training on ${NUM_OAM} OAM Modules"
...
if [[ ${PRECISION} == "bf16" ]]; then 
    ...
    echo "DLRM Training currently supports bf16 precision"
    exit 1
fi

cd ${MODEL_DIR}/models/recommendation/pytorch/torchrec_dlrm/training/gpu
I_MPI_DEBUG=6 ZE_AFFINITY_MASK=$ZE_AFFINITY_MASK mpirun -np $np -ppn $ppn --prepend-rank python -u dlrm_main.py ${ARGS} 2>&1 | tee ${OUTPUT_DIR}/dlrm_x${NUM_OAM}_training.log
