...
if [[ $PRECISION == "bf16" ]]; then
    NUM_BATCH=${NUM_BATCH} bash run_traininig.sh bf16 2>&1 | tee -a ${OUTPUT_DIR}/dlrm-training-convergence-bf16.log
elif [[ $PRECISION == "fp32" || $PRECISION == "avx-fp32" ]]; then
    NUM_BATCH=${NUM_BATCH} bash run_traininig.sh 2>&1 | tee -a ${OUTPUT_DIR}/dlrm-training-convergence-fp32.log
else
