...
if [[ "0" == ${TORCH_INDUCTOR} ]];then
    python -m intel_extension_for_pytorch.cpu.launch --nnodes ${NNODES} --hostfile ${HOSTFILE} --nprocs_per_node $SOCKETS --log_dir=${OUTPUT_DIR} --log_file_prefix="./throughput_log_phase1_${precision}" ${TRAIN_SCRIPT} \
