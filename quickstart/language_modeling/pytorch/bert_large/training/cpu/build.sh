...
DOCKERFILE=pytorch-spr-bert-large-training.Dockerfile
...
if [ "$(docker images -q ${PYTORCH_BASE_IMAGE}:${PYTORCH_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the BERT large training container."
...
docker build --build-arg PYTORCH_IMAGE=${PYTORCH_BASE_IMAGE} \
             ...
             --build-arg BERT_DIR=/workspace/pytorch-spr-bert-large-training/models/bert/bert \
