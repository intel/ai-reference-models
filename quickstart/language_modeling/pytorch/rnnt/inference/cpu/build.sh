...
DOCKERFILE=pytorch-spr-rnnt-inference.Dockerfile
PYTORCH_BASE_IMAGE=${PYTORCH_BASE_IMAGE:-model-zoo}
PYTORCH_BASE_TAG=${PYTORCH_BASE_TAG:-pytorch-ipex-spr}
IMAGE_NAME=${IMAGE_NAME:-model-zoo:pytorch-spr-rnnt-inference}
...
if [ "$(docker images -q ${PYTORCH_BASE_IMAGE}:${PYTORCH_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the RNN-T inference container."
