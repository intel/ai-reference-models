...
DOCKERFILE=tf-spr-bert-large-pretraining.Dockerfile
...
if [ "$(docker images -q ${TF_SPR_BASE_IMAGE}:${TF_SPR_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the BERT Large pretraining container."
