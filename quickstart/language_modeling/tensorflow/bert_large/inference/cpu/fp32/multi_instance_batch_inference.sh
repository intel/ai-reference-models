...
if [[ $? == 0 ]]; then
  ...
  grep 'throughput((num_processed_examples-threshold_examples)/Elapsedtime):' ${OUTPUT_DIR}/bert_large_fp32_inference_bs${BATCH_SIZE}_cores*_all_instances.log  | awk -F' ' '{sum+=$2;} END{print sum} '
else
  exit 1
fi
