...
if [[ ${PLATFORM} == "linux" ]]; then
    ...
    if [[ "$?" == 0 ]]; then
        ...
        --log_file_prefix="mnasnet1_0_latency_log_${PRECISION}""
...
if [[ ${PLATFORM} == "linux" ]]; then
  ...
  throughput=$(grep 'Throughput:' ${OUTPUT_DIR}/mnasnet1_0_latency_log_${PRECISION}_* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk -v INSTANCES_PER_SOCKET=$INSTANCES_THROUGHPUT_BENCHMARK_PER_SOCKET '
  BEGIN {
          sum = 0;
          i = 0;
        }
  ...
  echo "mnasnet1_0;"latency";${PRECISION};${BATCH_SIZE};${throughput}" | tee -a ${OUTPUT_DIR}/summary.log
fi
