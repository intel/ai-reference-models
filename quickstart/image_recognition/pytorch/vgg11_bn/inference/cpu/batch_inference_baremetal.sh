...
if [[ ${PLATFORM} == "linux" ]]; then
  CORES=`lscpu | grep Core | awk '{print $4}'`
    ...
    if [[ "$?" == 0 ]]; then
        ...
        --log_file_prefix="vgg11_bn_throughput_log_${PRECISION}""
...
throughput=$(grep 'Throughput:' ${OUTPUT_DIR}/vgg11_bn_throughput_log_${PRECISION}_* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk '
BEGIN {
        sum = 0;
        i = 0;
      }
...
echo "vgg11_bn;"throughput";${PRECISION};${BATCH_SIZE};${throughput}" | tee -a ${OUTPUT_DIR}/summary.log
