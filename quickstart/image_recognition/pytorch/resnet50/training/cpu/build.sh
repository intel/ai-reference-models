...
DOCKERFILE=pytorch-spr-resnet50-training.Dockerfile
...
if [ "$(docker images -q ${PYTORCH_BASE_IMAGE}:${PYTORCH_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the ResNet50 training container."
