...
if [[ ${PLATFORM} == "linux" ]]; then
    ...
    if [[ "$?" == 0 ]]; then
        MULTI_INSTANCE_ARGS=" -m intel_extension_for_pytorch.cpu.launch --memory-allocator jemalloc --latency_mode --log_path=${OUTPUT_DIR} --log_file_prefix="resnet101_latency_log_${PRECISION}""
...
if [[ ${PLATFORM} == "linux" ]]; then
  ...
  throughput=$(grep 'Throughput:' ${OUTPUT_DIR}/resnet101_latency_log_${PRECISION}_* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk -v INSTANCES_PER_SOCKET=$INSTANCES_THROUGHPUT_BENCHMARK_PER_SOCKET '
  BEGIN {
          sum = 0;
          i = 0;
        }
