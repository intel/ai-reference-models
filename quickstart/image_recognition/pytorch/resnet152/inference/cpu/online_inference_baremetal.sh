...
if [[ ${PLATFORM} == "linux" ]]; then
    ...
    if [[ "$?" == 0 ]]; then
        ...
        --log_file_prefix="resnet152_latency_log_${PRECISION}""
...
if [[ ${PLATFORM} == "linux" ]]; then
  ...
  throughput=$(grep 'Throughput:' ${OUTPUT_DIR}/resnet152_latency_log_${PRECISION}_* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk -v INSTANCES_PER_SOCKET=$INSTANCES_THROUGHPUT_BENCHMARK_PER_SOCKET '
  BEGIN {
          sum = 0;
          i = 0;
        }
  ...
  echo "resnet152;"latency";${PRECISION};${BATCH_SIZE};${throughput}" | tee -a ${OUTPUT_DIR}/summary.log
fi
