...
DOCKERFILE=pytorch-spr-resnext-32x16d-inference.Dockerfile
...
if [ "$(docker images -q ${PYTORCH_BASE_IMAGE}:${PYTORCH_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the ResNext 32x16d inference container."
