...
if [[ ${device_id} == "56c1" ]]; then
    ...
    do
            str+=("ZE_AFFINITY_MASK="${i}" python -u models/image_recognition/pytorch/resnet50v1_5/inference/gpu/main.py -a resnet50 -b ${BATCH_SIZE} --xpu 0 -e --pretrained --int8 1 --num-iterations ${NUM_ITERATIONS} --benchmark 1 ${dataset_arg} ")
    done
