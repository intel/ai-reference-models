...
if [[ ${PLATFORM} == "linux" ]]; then
  CORES=`lscpu | grep Core | awk '{print $4}'`
    ...
    if [[ "$?" == 0 ]]; then
        ...
        --memory-allocator jemalloc --log_path=${OUTPUT_DIR} --log_file_prefix="mnasnet0_5_accuracy_log_${PRECISION}""
...
if [[ ${PLATFORM} == "linux" ]]; then
  accuracy=$(grep 'Accuracy:' ${OUTPUT_DIR}/mnasnet0_5_accuracy_log_${PRECISION}_* |sed -e 's/.*Accuracy//;s/[^0-9.]//g')
  echo "mnasnet0_5;"accuracy";${PRECISION};${BATCH_SIZE};${accuracy}" | tee -a ${OUTPUT_DIR}/summary.log
fi
