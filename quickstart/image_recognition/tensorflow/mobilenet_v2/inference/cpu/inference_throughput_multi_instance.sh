...
_command python ${MODEL_DIR}/benchmarks/launch_benchmark.py \
  ...
  input_layer='input' output_layer='MobilenetV2/Predictions/Reshape_1'

if [[ $? == 0 ]]; then
  echo "Summary total images/sec:"
  grep 'Average Throughput:' ${OUTPUT_DIR}/mobilenet_v2_${PRECISION}_${MODE}_bs${BATCH_SIZE}_cores*_all_instances.log  | awk -F' ' '{sum+=$3;} END{print sum} '
  exit 0
else
  exit 1
fi
