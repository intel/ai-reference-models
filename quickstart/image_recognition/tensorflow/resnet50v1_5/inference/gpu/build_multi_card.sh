...
if [[ $GPU_TYPE == max-series ]];then
    ...
    IMAGE_NAME=${IMAGE_NAME:-intel/image-recognition:tf-flex-gpu-resnet50v1-5-multi-card-inference}
    TENSORFLOW_BASE_TAG=${TENSORFLOW_BASE_TAG:-gpu-flex}
    docker build \
    --build-arg PACKAGE_DIR=model_packages \
    --build-arg PACKAGE_NAME=tf-flex-series-resnet50v1-5-multi-card-inference \
    --build-arg MODEL_WORKSPACE=/workspace \
    --build-arg http_proxy=$http_proxy \
    --build-arg https_proxy=$https_proxy \
    --build-arg no_proxy=$no_proxy \
    ...
    --build-arg TENSORFLOW_BASE_TAG=${TENSORFLOW_BASE_TAG} \
    -t $IMAGE_NAME \
    -f tf-flex-series-resnet50v1-5-multi-card-inference.Dockerfile .
