...
if [ -z "${PRETRAINED_MODEL}" ]; then
  if [[ $PRECISION == "int8" ]]; then
      PRETRAINED_MODEL="${MODEL_DIR}/inceptionv3_int8_pretrained_model.pb"
      ARGS="$ARGS -- warmup_steps=50 steps=500"
  elif [[ $PRECISION == "fp32" ]]; then
      PRETRAINED_MODEL="${MODEL_DIR}/inceptionv3_fp32_pretrained_model.pb"
  else
      echo "The specified precision '${PRECISION}' is unsupported."
      echo "Supported precisions are: fp32 and int8"
      exit 1
