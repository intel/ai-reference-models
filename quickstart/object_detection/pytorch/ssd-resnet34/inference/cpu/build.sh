...
DOCKERFILE=pytorch-spr-ssd-resnet34-inference.Dockerfile
PYTORCH_BASE_IMAGE=${PYTORCH_BASE_IMAGE:-model-zoo}
PYTORCH_BASE_TAG=${PYTORCH_BASE_TAG:-pytorch-ipex-spr}
IMAGE_NAME=${IMAGE_NAME:-model-zoo:pytorch-spr-ssd-resnet34-inference}
...
if [ "$(docker images -q ${PYTORCH_BASE_IMAGE}:${PYTORCH_BASE_TAG})" == "" ]; then
  ...
  echo "This container is required, as it is used as the base for building the ssd-resnet34 inference container."
