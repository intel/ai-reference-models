...
if [[ "0" == ${TORCH_INDUCTOR} ]];then
	...
	--throughput_mode \
        ...
        --inductor \
        --seed 2024 \
        --batch-size $BATCH_SIZE 2>&1 | tee ${OUTPUT_DIR}/yolov7_throughput_log_${PRECISION}.log
fi

wait
cd -

throughput=$(grep 'Throughput:'  ${OUTPUT_DIR}/yolov7_throughput_log_${PRECISION}* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk '
BEGIN {
    sum = 0;
    i = 0;
    }
