...
IMAGE_NAME=${IMAGE_NAME:-intel/object-detection:pytorch-flex-gpu-ssd-mobilenet-multi-card-inference}

docker build \
    --build-arg PACKAGE_DIR=model_packages \
    --build-arg PACKAGE_NAME=pytorch-flex-series-ssd-mobilenet-multi-card-inference \
    ...
    -t $IMAGE_NAME \
    -f pytorch-flex-series-ssd-mobilenet-multi-card-inference.Dockerfile .
