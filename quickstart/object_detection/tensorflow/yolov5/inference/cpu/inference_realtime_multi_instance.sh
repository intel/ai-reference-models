...
if [[ $? == 0 ]]; then
  cat ${OUTPUT_DIR}/yolov5_${PRECISION}_inference_bs1_cores*_all_instances.log | grep 'Avg Throughput:' | sed -e s"/.*: //"
  echo "Throughput summary:"
  grep 'Avg Throughput' ${OUTPUT_DIR}/yolov5_${PRECISION}_inference_bs1_cores*_all_instances.log | awk -F' ' '{sum+=$3;} END{print sum} '
  exit 0
else
  exit 1
fi
