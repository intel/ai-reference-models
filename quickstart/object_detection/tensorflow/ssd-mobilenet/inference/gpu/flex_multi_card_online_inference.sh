...
mac_0=`lspci | grep Dis| head -n 1| awk '{print $1}'`
node_0=`lspci -s $mac_0 -v | grep NUMA | awk -F, '{print $5}' | awk '{print $3}'`
mac_1=`lspci | grep Dis| tail -n 1| awk '{print $1}'`
node_1=`lspci -s $mac_1 -v | grep NUMA | awk -F, '{print $5}' | awk '{print $3}'`
...
if [[ ${device_id} == "56c1" ]]; then
    ...
    "${str[@]}" 2>&1 | tee ${OUTPUT_DIR}//ssd-mobilenet_${PRECISION}_inf_block_c0_c1_${BATCH_SIZE}.log
    file_loc=${OUTPUT_DIR}//ssd-mobilenet_${PRECISION}_inf_block_c0_c1_${BATCH_SIZE}.log
