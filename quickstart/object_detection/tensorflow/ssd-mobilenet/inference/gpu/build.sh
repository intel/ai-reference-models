...
IMAGE_NAME=${IMAGE_NAME:-intel/object-detection:tf-flex-gpu-ssd-mobilenet-inference}

docker build \
    --build-arg PACKAGE_DIR=model_packages \
    --build-arg PACKAGE_NAME=tf-flex-series-ssd-mobilenet-inference \
    ...
    -t $IMAGE_NAME \
    -f tf-flex-series-ssd-mobilenet-inference.Dockerfile .
