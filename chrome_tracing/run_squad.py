...
flags.DEFINE_bool("profile", True, "[Optional] To enable Tensorflow profile hook."
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    if FLAGS.mode == 'benchmark':
      # add profiler hook to benchmark 
      hooks = [UpdateGlobalStepHook(),
               StopAtStepHook(total_steps),
               ProfilerHook(save_steps=total_steps-1, output_dir=FLAGS.output_dir, show_memory=True, show_dataflow=True)]
    ...
    if FLAGS.mode in ('benchmark', 'profile'):
      ...
      if FLAGS.predict_batch_size == 1:
        print('Latency (seconds) : ', (benchmark_duration / float(num_benchmark_examples)))
