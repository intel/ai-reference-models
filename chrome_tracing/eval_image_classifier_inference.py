  ...
  def run(self):
    ...
    if (not self.args.accuracy_only):
      ...
      while num_remaining_images >= self.args.batch_size and iteration < total_run:
        ...
        # run session with chrome trave options
        infer_sess.run([output_tensor], feed_dict={input_tensor: image_np},options=options,run_metadata=run_metadata)
      ...
      #filename to save chrome tracing json
      if (self.args.batch_size == 1):
      ...
      # save timelne object in chrome trace format
