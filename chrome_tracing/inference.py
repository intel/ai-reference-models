...
with tf.compat.v1.Session(config=config, graph=graph) as sess1:    
  ...
  while True:
    ...
    logistic = sess1.run(output_tensor, dict(zip(input_tensor, features_list[i][0:2])),options=options,run_metadata=run_metadata)
    if i > warm_iter:
  ...
  with open('trace_wide_deep_large_ds_v3.json', 'w') as trace_file:
