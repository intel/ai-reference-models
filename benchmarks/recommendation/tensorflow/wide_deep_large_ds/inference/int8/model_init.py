...
class ModelInitializer(BaseModelInitializer):
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            ...
            default="1",
            type=str,
            help="num of parallel batches.Default is 1",
        )
        parser.add_argument(
            "--kmp_block_time",
            dest="kmp_block_time",
        ...
        self.set_kmp_vars(
            config_file_path,
            kmp_settings=str(self.args.kmp_settings),
            kmp_blocktime=str(self.args.kmp_block_time),
            kmp_affinity=str(self.args.kmp_affinity),
        )
    ...
    def run_benchmark(self):
        ...
        if enable_parallel_batches == "True":
            benchmark_script = os.path.join(
                self.args.intelai_models, self.args.mode, "parallel_inference.py"
            )
            script_args_list.append("num_parallel_batches")
        else:
            benchmark_script = os.path.join(
                self.args.intelai_models, self.args.mode, "inference.py"
            )
        command_prefix = self.get_command_prefix(-1)
        if self.args.socket_id != -1 and self.args.num_cores != -1:
            command_prefix = (
                command_prefix
                + " numactl --physcpubind=0-{} --membind={} ".format(
                    str(int(self.args.num_cores) - 1), self.args.socket_id
                )
            )
