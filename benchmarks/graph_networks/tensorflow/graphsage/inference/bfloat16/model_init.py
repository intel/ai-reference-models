...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args=[], platform_util=None):
        ...
        arg_parser.add_argument(
            "--steps", dest="steps", type=int, default=5524, help="number of steps"
        )
        ...
        benchmark_script = os.path.join(
            self.args.intelai_models, self.args.mode, "eval_ppi_graphsage.py"
        )
        ...
        if self.args.pretrained_model and os.listdir(self.args.pretrained_model):
            self.benchmark_command += (
                " --pretrained-model=" + self.args.pretrained_model
            )
