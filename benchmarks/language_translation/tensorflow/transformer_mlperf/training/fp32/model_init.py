...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args, platform_util=None):
        ...
        run_script = os.path.join(
            ...
            self.args.mode,
            "fp32/transformer/transformer_main.py",
        )
        ...
        cmd_args = (
            " --data_dir="
            + str(self.args.data_location)
            + " --random_seed="
            + str(self.args.random_seed)
            + " --params="
            + str(self.args.params)
            + " --train_steps="
            + str(self.args.train_steps)
            + " --steps_between_eval="
            + str(self.args.steps_between_eval)
            + " --do_eval="
            + str(self.args.do_eval)
            + " --save_checkpoints="
            + str(self.args.save_checkpoints)
            + " --save_profile="
            + str(self.args.save_profile)
            + " --print_iter="
            + str(self.args.print_iter)
