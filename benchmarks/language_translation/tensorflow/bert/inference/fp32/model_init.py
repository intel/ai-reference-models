...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args, platform_util):
        ...
        parser.add_argument(
            "--task_name", type=str, default="XNLI", dest="task_name", help="take name"
        )
        parser.add_argument(
            "--max_seq_length",
            type=int,
        ...
        self.run_cmd = self.get_command_prefix(self.args.socket_id) + "{} {}".format(
            self.python_exe, script_path
        )
        self.run_cmd += " --data_dir={} --output_dir={}".format(
            self.args.data_location, format(self.args.output_dir)
        )
        ...
        self.run_cmd += (
            " --vocab_file={} --bert_config_file={} --init_checkpoint={}".format(
        ...
        if self.args.accuracy_only:
            self.run_cmd += (
                " --do_train=true --num_train_epochs=3.0 --train_batch_size=32"
            )
        else:
