    ...
    if value is not None:
        if not os.path.isfile(value) and not os.path.isdir(value):
            raise argparse.ArgumentTypeError(
                "{} does not exist or is not a file/folder.".format(value)
            )
        ...
        if (i + 1) % batch_size == 0 or i == sentences_to_translate - 1:
            ...
            request.inputs["input"].CopyFrom(
                tf.make_tensor_proto(
                    pd.DataFrame(batch).fillna(0).values.astype(np.int64)
                )
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-d",
        "--data_file",
        type=check_valid_file_or_folder,
        required=True,
    ...
    print(
        ...
        "num_iteration={}, warm_up_iteration={}".format(
            BATCH_SIZE, NUM_ITERATION, WARM_UP_ITERATION
        )
    )
    benchmark(
