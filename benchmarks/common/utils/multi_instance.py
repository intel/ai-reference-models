...
def buckets(array: List[Any], bucket_size: int) -> List[List[Any]]:
    """
    ...
    :rtype: List[List[Any]]
    """
    bucket_size_list: List[List[Any]] = []
...
class InferencePrefix:
    ...
    def __init__(self, sockets: int = 0, instances: int = 0, cores_per_instance: int = 0) -> None:
        """
    ...
    @property
    def is_basic_configuration(self) -> bool:
        """
    ...
    @property
    def instances_per_socket(self) -> int:
        """
        ...
        if self._instances > 0:
            if self._instances % self.sockets != 0:
                raise Exception("Instances could not be distributed equally between sockets. "
                                ...
                                f"{self._instances} % {self.sockets} != 0")
            ...
            if self._cores_per_instance > 0:
                if self._cores_per_instance * self.instances_per_socket > self.cores_per_socket:
                    raise Exception("Total cores used on one socket > cores available on one socket. "
                                    f"{self._cores_per_instance} * {self.instances_per_socket} "
    ...
    @property
    def sockets_per_instance(self) -> int:
        """
    ...
    @staticmethod
    def get_cores_range(cores: Optional[str], ht_cores: Optional[str], use_ht: bool) -> Union[str, Any]:
        """
        ...
        if use_ht and ht_cores:
            cores_range: Optional[str] = "{},{}".format(cores, ht_cores)
        else:
            cores_range = cores

        return cores_range

    def split_cores(self) -> Dict[str, List[List[Dict[str, Any]]]]:
        """
    ...
    def generate_multi_instance_ranges(self, use_ht: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        for instance_buckets in split_cores.values():
            for instance_config in instance_buckets:
                if len(instance_config) == 1:
                    cores: Optional[str] = instance_config[0].get("cpu_id")
                    ht_cores: Optional[str] = instance_config[0].get("ht_cpu_id", None)
                else:
    ...
    def generate_multi_instance_prefix(self, command: list, use_ht: bool = False) -> List[list]:
        """
    ...
    def _platform_single_instance_args(self) -> Dict[str, Optional[int]]:
        """
