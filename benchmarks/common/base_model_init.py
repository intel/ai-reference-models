...
class BaseModelInitializer(object):
    ...
    def run_command(self, cmd, replace_unique_output_dir=None):
        ...
        if self.args.numa_cores_per_instance:
            ...
            if not num_numas:
                print(
                    "Warning: Unable to run multiple instances using numactl, "
    ...
    def run_numactl_multi_instance(self, cmd, replace_unique_output_dir=None):
        ...
        if self.args.numa_cores_per_instance != "socket":
            ...
            if self.args.socket_id != -1:
                ...
                if len(cpu_cores_list) > self.args.socket_id:
                    ...
                    raise ValueError(
                        "Error while trying to get the core list for socket {0}. "
                        "The core list does not have cores for socket {0}.\n "
                        "Core list: {1}\n".format(
                            self.args.socket_id, str(cpu_cores_list)
                        )
                    )
            else:
            ...
            if self.args.socket_id != -1:
                ...
                instance_cores_list[0] = self.platform_util.cpu_core_list[
                    self.args.socket_id
                ]
            else:
        ...
        for instance_num, core_list in enumerate(instance_cores_list):
            if cores_per_instance != "socket" and len(core_list) < int(
                cores_per_instance
            ):
                print(
                    "NOTE: Skipping remainder of {} cores for instance {}".format(
                        len(core_list), instance_num
                    )
                )
                continue
            ...
            if "OMP_NUM_THREADS" in os.environ:
                prefix = (
                    "{0}OMP_NUM_THREADS={1} " "numactl --localalloc --physcpubind={2}"
                ).format(
                    ld_preload_prefix,
                ...
                prefix = (
                    "{0}OMP_NUM_THREADS={1} " "numactl --localalloc --physcpubind={2}"
                ).format(ld_preload_prefix, len(core_list), ",".join(core_list))
            instance_logfile = log_filename_format.format(
            ...
            if replace_unique_output_dir:
                ...
                unique_dir = os.path.join(
                    replace_unique_output_dir, "instance_{}".format(instance_num)
                )
                unique_command = unique_command.replace(
                    replace_unique_output_dir, unique_dir
                )
        ...
        while retry_counter < max_retries:
            ...
            if retry_counter >= max_retries:
                print(
                    "Warning: Log files for all instances were not found after "
        ...
        with open(all_instance_log, mode="w") as combined_file:
            for instance_logfile in instance_logfiles:
                if not os.path.exists(instance_logfile):
                    print(
                        "Skipping {} when generating the combined log file, because "
        ...
        print(
            "\n".join(
                [
                    ...
                    for log_path in instance_logfiles
    ...
    def get_command_prefix(self, socket_id, numactl=True):
        ...
        if not self.args.disable_tcmalloc:
            ...
            if len(matches) > 0:
                ...
                print(
                    "Warning: Unable to find the TCMalloc library file (libtcmalloc.so) in /usr/lib, /usr/lib64, "
        ...
        if (
            ...
            and socket_id != -1
            and numactl
            and not self.args.numa_cores_per_instance
        ):
            if self.args.num_cores == -1:
                ...
                for i in range(num_sockets):
                    if num_cores_in_socket0 != len(self.platform_util.cpuset_cpus[i]):
                        raise ValueError(
        ...
        * num_intra_threads = number of cores per instance
        ...
        if self.args.numa_cores_per_instance:
            ...
            if self.args.numa_cores_per_instance == "socket":
                if self.args.socket_id != -1:
                    inter_threads = len(
                        self.platform_util.cpu_core_list[self.args.socket_id]
                    )
                else:
                    ...
                    inter_threads = min(
                        [len(i) for i in self.platform_util.cpu_core_list if len(i) > 0]
                    )
            else:
            ...
            if not self.args.num_intra_threads:
                if self.args.num_cores != -1:
                    ...
                    self.platform_util.cpuset_cpus
                    and self.args.socket_id in self.platform_util.cpuset_cpus.keys()
                ):
                    self.args.num_intra_threads = len(
                        self.platform_util.cpuset_cpus[self.args.socket_id]
                    )
                else:
            ...
            if not self.args.num_intra_threads:
                if self.args.num_cores == -1:
                    if (
                        self.platform_util.cpuset_cpus
                        and len(self.platform_util.cpuset_cpus.keys()) > 0
                    ):
                        # Total up the number of cores in the cpuset
                        self.args.num_intra_threads = sum(
                            [
                                len(self.platform_util.cpuset_cpus[socket_id])
