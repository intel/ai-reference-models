...
class BaseBenchmarkUtil(object):
    ...
    def _define_args(self):
        ...
        self._common_arg_parser = ArgumentParser(
            add_help=False, description="Parse args for base benchmark " "interface"
        )
        ...
        self._common_arg_parser.add_argument(
            ...
            "--model-name",
            required=required_arg,
            help="model name to run benchmarks for",
            dest="model_name",
        )
        ...
        self._common_arg_parser.add_argument(
            "--numa-cores-per-instance",
            type=check_num_cores_per_instance,
        ...
        self._common_arg_parser.add_argument(
            ...
            dest="tcmalloc_large_alloc_report_threshold",
            default=2147483648,
            type=int,
        )
    ...
    def _validate_args(self):
        ...
        args = self.args
        if args.weight_sharing is True and (
            args.model_name != "resnet50v1_5" or args.precision == "fp32"
        ):
            print(
                "Warning: Weight sharing support available only for RN50 int8 and bfloat16 models"
            )
        if not -1 <= args.socket_id < num_numas:
            if num_numas > 0:
                raise ValueError(
                    "Socket id must be within NUMA number range: "
                    "[0, {}].".format(num_numas - 1)
                )
            else:
                print(
                    "Warning: There are no NUMA nodes on your system and a socket id has "
        ...
        system_num_cores = (
            ...
            if num_numas and args.socket_id != -1
            else self._platform_util.num_cores_per_socket
        ...
        if (num_cores <= 0) and (num_cores != -1):
            ...
            raise ValueError(
                "Number of cores exceeds system core number: {}".format(
                    system_num_cores
                )
            )

        if args.output_results and (
            (args.model_name != "resnet50" and args.model_name != "resnet50v1_5")
            or (args.precision != "fp32" and args.precision != "fp16")
        ):
            raise ValueError(
                "--output-results is currently only supported for resnet50 FP32 or FP16 inference."
            )
        elif args.output_results and (
            args.mode != "inference" or not args.data_location
        ):
            raise ValueError(
                "--output-results can only be used when running inference with a dataset."
            )

        if args.num_instances != 1:
            print(
                "Warning: The --num-instances flag is deprecated and will be removed in the future. "
        ...
        if args.numa_cores_per_instance:
            ...
            if args.mpi:
                raise ValueError(
                    "--mpi_num_processes cannot be used together with --numa-cores-per-instance."
                )

            if args.numa_cores_per_instance != "socket":
                if args.socket_id != -1:
                    if (
                        int(args.numa_cores_per_instance)
                        > self._platform_util.num_cores_per_socket
                    ):
                        raise ValueError(
                            "The number of --numa-cores-per-instance ({}) cannot exceed the "
                            "number of cores per socket {} when a single socket (--socket-id {}) "
                            "is being used.".format(
                                args.numa_cores_per_instance,
                                self._platform_util.num_cores_per_socket,
                                args.socket_id,
                            )
                        )
                    ...
                    if int(args.numa_cores_per_instance) > system_num_cores:
                        raise ValueError(
                            "The number of --numa-cores-per-instance ({}) cannot exceed the "
                            "number of system cores ({}).".format(
                                args.numa_cores_per_instance, system_num_cores
                            )
                        )
        ...
        if args.socket_id != -1 and self._platform_util.cpuset_cpus:
            ...
            if args.socket_id in self._platform_util.cpuset_cpus.keys():
                cpuset_len_for_socket = len(
                    self._platform_util.cpuset_cpus[args.socket_id]
                )

            if cpuset_len_for_socket == 0:
                sys.exit(
                    "ERROR: There are no socket id {} cores in the cpuset.".format(
                        args.socket_id
                    )
                )
            elif cpuset_len_for_socket < self._platform_util.num_cores_per_socket:
                print(
                    "Note: Socket id {} is specified, but the cpuset has limited this socket to {} cores. "
                    "This is less than the number of cores per socket on the system ({})".format(
                        args.socket_id,
                        cpuset_len_for_socket,
                        self._platform_util.num_cores_per_socket,
                    )
                )
        ...
        if args.gpu:
            ...
            if args.num_intra_threads is not None:
                raise ValueError(
                    "--num-intra-threads cannot be used with --gpu parameter."
                )
            if args.num_inter_threads is not None:
                raise ValueError(
                    "--num-inter-threads cannot be used with --gpu parameter."
                )
        ...
        if args.model_name:  # not empty
            ...
            if args.numa_cores_per_instance == "socket":
                if self._platform_util.cpuset_cpus:
                    if args.socket_id != -1:
                        args.numa_cores_per_instance = len(
                            self._platform_util.cpuset_cpus[args.socket_id]
                        )
                    else:
