...
void InitializeReader(std::unique_ptr<DataReader> &reader,
                      bool convert_to_rgb) {
  reader.reset(new DataReader(FLAGS_data_list, FLAGS_data_dir,
...
bool ReadNextBatch(PaddleTensor &input_data, PaddleTensor &input_labels,
  ...
  return reader->NextBatch(
      static_cast<float *>(input_data.data.data()),
      FLAGS_with_labels ? static_cast<int64_t *>(input_labels.data.data())
                        : nullptr,
