...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args=[], platform_util=None):
        ...
        arg_parser.add_argument(
            "--infer-option",
            help=" Inference SQuAD, Pretraining or classifier",
            dest="infer_option",
            default="SQuAD",
        )
        arg_parser.add_argument(
        ...
        if self.args.vocab_file and not os.path.isabs(self.args.vocab_file):
            self.args.vocab_file = os.path.join(
                self.args.data_location, self.args.vocab_file
            )

        if self.args.bert_config_file and not os.path.isabs(self.args.bert_config_file):
            self.args.bert_config_file = os.path.join(
                self.args.data_location, self.args.bert_config_file
            )

        if self.args.predict_file and not os.path.isabs(self.args.predict_file):
            self.args.predict_file = os.path.join(
                self.args.data_location, self.args.predict_file
            )

        if self.args.init_checkpoint and not os.path.isabs(self.args.init_checkpoint):
            self.args.init_checkpoint = os.path.join(
                self.args.checkpoint, self.args.init_checkpoint
            )
        ...
        if self.args.accuracy_only:
            ...
            if os.path.exists(predictions_json):
                evaluate_cmd = "{} {} {} {}".format(
                    self.python_exe,
                    evaluate_script,
                    self.args.predict_file,
                    predictions_json,
                )
                os.system(evaluate_cmd)
            else:
                print(
                    "Warning: The {} preditions file was not found. Unable to "
