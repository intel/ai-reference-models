...
```
$ mkdir -p pre-trained && cd pre-trained
$ wget https://storage.cloud.google.com/intel-optimized-tensorflow-staging/models/ssdvgg16_fp32_pretrained_model.pb
$ pwd
...
For Latency, use `--batch-size 1`, `--single-socket`
```
python launch_benchmark.py \
    --platform fp32 \
    --model-name ssd-vgg16 \
...
```
For Throughput, `--batch-size 224`, `--single-socket`
```
python launch_benchmark.py \
    --platform fp32 \
    --model-name ssd-vgg16 \
