...
class ModelInitializer(BaseModelInitializer):
    def run_inference_sanity_checks(self, args, custom_args):
        if not args.input_graph:
            sys.exit(
                "Please provide a path to the frozen graph directory"
                " via the '--in-graph' flag."
            )
        if not args.data_location and self.args.accuracy_only:
            sys.exit(
                "Please provide a path to the data directory via the "
                "'--data-location' flag."
            )
        if args.socket_id == -1 and args.num_cores == -1:
            print(
                "***Warning***: Running inference on all cores could degrade"
    ...
    def run(self):
        ...
        benchmarks_path = os.path.join(
            self.args.model_source_dir, "../ssd-resnet-benchmarks"
        )
        os.environ["PYTHONPATH"] = os.path.join(self.args.model_source_dir, "research")
        os.environ["PYTHONPATH"] += ":" + os.path.join(
            benchmarks_path, "scripts/tf_cnn_benchmarks"
        )
