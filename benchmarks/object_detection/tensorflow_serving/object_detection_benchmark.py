...
def make_request(batch_size):
    if PROTOCOL == 'rest':
        np_images = np.repeat(
            np.expand_dims(get_random_image(IMAGES_PATH), 0).tolist(), batch_size, axis=0).tolist()
...
if __name__ == '__main__':
    ...
    print('\nStarting {} model benchmarking for latency on {}:'.
          format(MODEL.upper(), PROTOCOL.upper()))
    print('batch_size=1, num_iteration=20, warm_up_iteration=10\n')
    benchmark(batch_size=1, num_iteration=20, warm_up_iteration=10)

    print('\nStarting {} model benchmarking for throughput on {}:'.
          format(MODEL.upper(), PROTOCOL.upper()))
