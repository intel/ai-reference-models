...
def check_valid_model(value):
    ...
    if value not in ("rfcn", "ssd-mobilenet"):
        raise argparse.ArgumentError(
            "Model name {} does not match 'rfcn' or 'ssd-mobilenet'.".format(value)
        )
    return value
...
def check_valid_protocol(value):
    ...
    if value not in ("rest", "grpc"):
        raise argparse.ArgumentError(
            "Protocol name {} does not match 'rest' or 'grpc'.".format(value)
        )
    return value
...
def make_request(batch_size):
    if PROTOCOL == "rest":
        np_images = np.repeat(
            np.expand_dims(get_random_image(IMAGES_PATH), 0).tolist(),
            batch_size,
            axis=0,
        ).tolist()
        ...
        np_images = np.repeat(
            np.expand_dims(get_random_image(IMAGES_PATH), 0), batch_size, axis=0
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-p",
        "--protocol",
        type=check_valid_protocol,
        required=False,
        default="grpc",
        help="Name of protocol (rest or grpc)",
    )
    ap.add_argument("-b", "--batch_size", type=int, required=True, help="Batch size")
    ...
    print(
        "\nStarting {} model benchmarking for latency on {}:".format(
            MODEL.upper(), PROTOCOL.upper()
        )
    )
