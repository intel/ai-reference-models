...
class RunCmd:
    ...
    def runJupyterNotebook(
        self, input_notebook_filename, output_notebook_filename, conda_env, fdpath="./"
    ):
        import nbformat
        import os
        ...
        try:
            with open(input_notebook_filename) as f:
                ...
                ep = ExecutePreprocessor(
                    timeout=6000, kernel_name=conda_env, allow_errors=True
                )
    ...
    def runPerfComparisonCmd(
        ...
        cmd = self.runJupyterNotebook(
            "benchmark_perf_comparison.ipynb",
            "profiling/unit_test_results/benchmark_perf_comparison_"
            + postfix
            + ".ipynb",
            conda_env,
        )
    ...
    def runPerfTimelneCmd(
        self, conda_env, postfix, fd_inex="0", file_index="0", baseline_index="0"
    ):
        import os
        ...
        cmd = self.runJupyterNotebook(
            ...
            "profiling/unit_test_results/benchmark_perf_timeline_analysis_"
            + postfix
            + ".ipynb",
            conda_env,
        )
    ...
    def runDataTypeCmd(
        ...
        os.environ["DATA_DOWNLOAD_PATH"] = data_download_path
        cmd = self.runJupyterNotebook(
            "benchmark_data_types_perf_comparison.ipynb",
            "profiling/unit_test_results/benchmark_data_types_perf_comparison_"
            + postfix
            + ".ipynb",
            conda_env,
        )
    ...
    def runDataTypeTimelneCmd(
        ...
        cmd = self.runJupyterNotebook(
            "benchmark_data_types_perf_timeline_analysis.ipynb",
            "profiling/unit_test_results/benchmark_data_types_perf_timeline_analysis_"
            + postfix
            + ".ipynb",
            conda_env,
        )
...
class Utils:
    ...
    def mvCSV2Results(self, folder=None):
        ...
        for f in csv_paths:
            if os.path.isfile(f) is True:
                ...
                print(model)
                self.log_model_speedupcsv(
                    model,
                    perf_ratio,
                    mean1,
                    mean2,
                    "profiling/unit_test_results/unit_test_summary.csv",
                )
                shutil.move(f, unit_test_dir)
    ...
    def get_perf_ratio_from_csv(self, filepath, framename):
        ...
        perfp = PerfPresenter()
        mean1, mean2, len1, len2, stdev1, stdev2 = perfp.read_number_from_csv(
            filepath, framename
        )
    ...
    def log_model_speedupcsv(self, model, speedup, mean1, mean2, filename):
        ...
        with f:
            ...
            writer = csv.DictWriter(f, fieldnames=fnames)
            writer.writerow(
                {"model": model, "speedup": speedup, "stock": mean2, "intel": mean1}
            )
    ...
    def test_patches(self, topology_name, ModelZooRoot=""):
        ...
        ret2 = config.unpatch_model_to_enable_timeline(
            model_path=ModelZooRoot + "/models/"
        )
        ret = ret1 or ret2
...
class TestPerfComparison(unittest.TestCase):
    def setUp(self):
        ...
        self.TimelineEnabled = config.read_value_from_section(
            test_name, "TimelineEnabled"
        )
        self.OnlineEnabled = config.read_value_from_section(test_name, "OnlineEnabled")
        self.baseline_conda = config.read_value_from_section(
            test_name, "baseline_conda"
        )
        self.compare_conda = config.read_value_from_section(test_name, "compare_conda")

        self.rfcn_infer_fp32_model_path = config.read_value_from_section(
            test_name, "rfcn_infer_fp32_model_path"
        )
        self.rfcn_infer_fp32_data_path = config.read_value_from_section(
            test_name, "rfcn_infer_fp32_data_path"
        )
        self.mobilenet_v1_infer_fp32_model_path = config.read_value_from_section(
            test_name, "mobilenet_v1_infer_fp32_model_path"
        )
        self.wide_deep_large_infer_fp32_data_path = config.read_value_from_section(
            test_name, "wide_deep_large_infer_fp32_data_path"
        )
        self.bert_infer_fp32_data_path = config.read_value_from_section(
            test_name, "bert_infer_fp32_data_path"
        )
        self.bert_infer_fp32_model_path = config.read_value_from_section(
            test_name, "bert_infer_fp32_model_path"
        )
        self.transformer_mlperf_train_fp32_data_path = config.read_value_from_section(
            test_name, "transformer_mlperf_train_fp32_data_path"
        )
        self.ssd_mobilenet_infer_fp32_data_path = config.read_value_from_section(
            test_name, "ssd_mobilenet_infer_fp32_data_path"
        )
    ...
    def test_0_env_setup(self):
        ...
        if setup is True:
            cmd1 = (
                "conda create -n "
                + self.baseline_conda
                + " python matplotlib ipykernel psutil pandas gitpython"
            )
            ret1 = RunCmd().run(cmd1)
            cmd2 = (
                "conda run -n "
                + self.baseline_conda
                + " pip install tensorflow cxxfilt"
            )
            ...
            cmd5 = (
                "conda run -n "
                + self.compare_conda
                + " pip install intel-tensorflow cxxfilt"
            )
    ...
    def test_a_resnet50_infer_fp32(self):
        ...
        topo_index = "0"
        ret1 = RunCmd().runPerfComparisonCmd(
            self.baseline_conda,
            "False",
            topo_index,
            "True",
            postfix,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
            "False",
            topo_index,
            "True",
    ...
    def test_g_rfcn_infer_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            model_source_dir=self.rfcn_infer_fp32_model_path,
            data_download_path=self.rfcn_infer_fp32_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
    ...
    def test_j_mobilenet_v1_infer_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            model_source_dir=self.mobilenet_v1_infer_fp32_model_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
    ...
    def test_l_wide_deep_large_infer_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            data_download_path=self.wide_deep_large_infer_fp32_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
    ...
    def test_m_bert_infer_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            model_source_dir=self.bert_infer_fp32_model_path,
            data_download_path=self.bert_infer_fp32_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
    ...
    def test_n_transformer_mlperf_train_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            data_download_path=self.transformer_mlperf_train_fp32_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
    ...
    def test_o_ssd_mobilenet_infer_fp32(self):
        ...
        ret1 = RunCmd().runPerfComparisonCmd(
            ...
            "True",
            postfix,
            data_download_path=self.ssd_mobilenet_infer_fp32_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret2 = RunCmd().runPerfComparisonCmd(
            self.compare_conda,
...
class TestPerfDataType(unittest.TestCase):
    def setUp(self):
        ...
        self.mobilenet_v1_infer_model_path = config.read_value_from_section(
            test_name, "mobilenet_v1_infer_model_path"
        )
        self.rfcn_infer_model_path = config.read_value_from_section(
            test_name, "rfcn_infer_model_path"
        )
        self.rfcn_infer_data_path = config.read_value_from_section(
            test_name, "rfcn_infer_data_path"
        )
        self.ssdmobilenet_data_path = config.read_value_from_section(
            test_name, "ssdmobilenet_data_path"
        )
        self.wide_deep_large_ds_infer_data_path = config.read_value_from_section(
            test_name, "wide_deep_large_ds_infer_data_path"
        )
        self.transformer_mlperf_training_data_path = config.read_value_from_section(
            test_name, "transformer_mlperf_training_data_path"
        )
        Utils().createFolderForResults()
    ...
    def test_a_resnet50v1_5_infer(self):
        ...
        ret = 0
        data_type_index = 0
        cmd_ret = RunCmd().runDataTypeCmd(
            self.compare_conda,
            "False",
            topo_index,
            data_type_index,
            self.TimelineEnabled,
            postfix,
            enable_online=self.OnlineEnabled,
        )
        ret = ret or cmd_ret
        ...
        cmd_ret = RunCmd().runDataTypeTimelneCmd(
            self.compare_conda, postfix, baseline_index="1", comparison_index="2"
        )
        ret = ret or cmd_ret
    ...
    def test_f_rfcn_infer(self):
        ...
        cmd_ret = RunCmd().runDataTypeCmd(
            ...
            self.TimelineEnabled,
            postfix,
            model_source_dir=self.rfcn_infer_model_path,
            data_download_path=self.rfcn_infer_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret = ret or cmd_ret
    ...
    def test_g_ssdmobilenet_infer(self):
        ...
        cmd_ret = RunCmd().runDataTypeCmd(
            ...
            self.TimelineEnabled,
            postfix,
            data_download_path=self.ssdmobilenet_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret = ret or cmd_ret
    ...
    def test_i_transformer_mlperf_training(self):
        ...
        cmd_ret = RunCmd().runDataTypeCmd(
            ...
            self.TimelineEnabled,
            postfix,
            data_download_path=self.transformer_mlperf_training_data_path,
            enable_online=self.OnlineEnabled,
        )
        ret = ret or cmd_ret
        ...
        cmd_ret = RunCmd().runDataTypeTimelneCmd(
            self.compare_conda, postfix, baseline_index="1", comparison_index="0"
        )
        ret = ret or cmd_ret
        self.assertEqual(ret, 0)
