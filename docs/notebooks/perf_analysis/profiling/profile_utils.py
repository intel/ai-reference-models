...
class CSVCommonUtils:
    ...
    def write_to_csv(self, dataframes):
        ...
        with f:
            writer = csv.DictWriter(f, fieldnames=self.fnames)
            writer.writerow(
                {self.fnames[0]: dataframes[0], self.fnames[1]: dataframes[1]}
            )
    ...
    def get_tf_ops_time(self, timeline_pd, fn, tfile_prefix):
        ...
        if total_mkl_time > 0.0:
            ...
            with f:
                ...
                writer.writerow(
                    {"op": "nono_mkl_op", "time": total_time - total_mkl_time}
                )
    ...
    def merge_two_csv_files_v2(self, merged_filepaths, a, b, tags=["stock", "intel"]):
        ...
        merged = merged.rename(
            columns={
                ...
                merged.columns.values[5]: tags[1],
            }
        )
        ...
        merged = merged.drop(
            columns=[
                ...
                merged.columns.values[4],
                merged.columns.values[6],
                merged.columns.values[7],
                merged.columns.values[8],
            ]
        )
        if self.showAbsNumber is False:
            ret = merged.drop(
                columns=[merged.columns.values[1], merged.columns.values[5]]
            )
        else:
            ret = merged
