...
def get_args():
    ...
    parser.add_argument(
        "--vocab-file", default="vocab.txt", type=str, help="Path to the vocab.txt file"
    )
    parser.add_argument(
        "--tsv-dir",
        default="imdb_tsv",
    ...
    parser.add_argument(
        "--max-seq-length", default=128, type=int, help="Max sequence length"
    )
...
def validate_args(args):
    ...
    if not os.path.exists(args.config_yaml):
        raise ValueError(
            "The specified config yaml ({}) does not exist".format(args.config_yaml)
        )

    if not os.path.exists(args.vocab_file):
        raise ValueError(
            "The specified vocab file ({}) does not exist.".format(args.vocab_file)
        )

    if not os.path.isdir(args.tsv_dir):
        raise ValueError(
            "The specified tsv dir ({}) does not exist.".format(args.tsv_dir)
        )

    if not os.path.isdir(args.output_dir):
        raise ValueError(
            "The specified output directory ({}) does not exist.".format(
                args.output_dir
            )
        )
...
class TF_BERTDataSet:
    ...
    def __getitem__(self, idx):
        ...
        return (
            np.array(input_mask_data).astype("int32"),
