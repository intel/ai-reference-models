...
if [[ "${PLATFORM}" == "Max" ]]; then
    ...
    if [[ "${PRECISION}" != "FP16" ]]; then
      echo "Swin Transformer currently supports FP16 precision" 
      exit 1
		fi
...
if [[ "${MULTI_TILE}" == "True" ]]; then
    echo "Only support MULT_TILE=False"
    exit 1
fi
...
echo "Swin-transformer ${PRECISION} inference plain MultiTile=${MULTI_TILE} BS=${BATCH_SIZE} Iter=${NUM_ITERATIONS}"

wget -qc https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224.pth

cd swim_transformer_data
wget -qc https://github.com/microsoft/Swin-Transformer/blob/main/data/map22kto1k.txt
cd ..
...
if [[ ${MULTI_TILE} == "False" ]]; then
    ...
    IPEX_XPU_ONEDNN_LAYOUT=0 python -u main_no_ddp.py --eval --cfg configs/swin/swin_base_patch4_window7_224.yaml --resume swin_base_patch4_window7_224.pth --data-path ${DATASET_DIR} --local_rank 1 --batch-size ${BATCH_SIZE} --num-iterations ${NUM_ITERATIONS} 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
