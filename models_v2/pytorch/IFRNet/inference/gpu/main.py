...
try:
    ...
    raise ImportError("Failed to import IFRNet model.\n"
                      "Pls set PYTHONPATH env variable to include path to the model.")

def load_model(args):
    # Load IFRNet  model, with specified pretrained weights`
    model = Model()
    if args.device.startswith('xpu') and not args.ipex:
        # map_location does not currently work on native XPU backend
        pretrained_weights = torch.load(args.pretrained_weights)
    else:
        pretrained_weights = torch.load(args.pretrained_weights, map_location=args.device)
    ...
    dtype, _ = ifrnet_util.get_data_type(args.precision, args.amp)
    if not args.amp:
        model = model.to(dtype)
    model = model.to(args.device)
    io_utils.write_info("Loaded IFRNet model.")
...
def process_ifrnet_interpolation(args, instance):
    args.instance = instance
    ifrnet_util.args = args
    ...
    interp_handle = ifrnet_util.IFRInterpolation(model, args, "IFRNet", instance)
...
def main():
    ...
    ifrnet_util.parse_arguments()
    ifrnet_util.show_arguments()
    args = ifrnet_util.args
    ...
    for instance in range(1, args.streams + 1):
        processes[instance] = mp.Process(target=process_ifrnet_interpolation, args=(args, instance))
