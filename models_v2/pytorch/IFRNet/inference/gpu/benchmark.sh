...
PROFILE=${PROFILE:-profiles/IFRNet.fp16.csv}

PLATFORM=${PLATFORM:-Flex}
AMP=${AMP:-no}

if [[ ${PLATFORM} == "Flex" || ${PLATFORM} == "Max" ]]; then
  IMAGE=${IMAGE:-intel/image-interpolation:pytorch-flex-gpu-ifrnet}
  platform_opt="--device /dev/dri/"
elif [[ ${PLATFORM} == "CUDA" ]]; then
  IMAGE=${IMAGE:-intel/image-interpolation:pytorch-cuda-gpu-ifrnet}
