...
PROFILE=${PROFILE:-profiles/yolov5m.fp16.csv}

PLATFORM=${PLATFORM:-Flex}
AMP=${AMP:-no}

if [[ ${PLATFORM} == "Flex" || ${PLATFORM} == "Max" ]]; then
  IMAGE=${IMAGE:-intel/object-detection:pytorch-flex-gpu-yolov5-inference}
  platform_opt="--device /dev/dri/"
elif [[ ${PLATFORM} == "CUDA" ]]; then
  IMAGE=${IMAGE:-intel/object-detection:pytorch-cuda-gpu-yolov5-inference}
