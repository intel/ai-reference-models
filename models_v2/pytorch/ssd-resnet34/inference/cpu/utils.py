...
### This file is originally from: [mlcommons repo](https://github.com/mlcommons/inference/tree/r0.5/others/cloud/single_stage_detector/pytorch/utils.py)
import torch
import torchvision
...
class Encoder(object):
    ...
    def decode_batch(
        self, bboxes_in, scores_in, criteria=0.45, max_output=200, device=0
    ):
        if use_ipex:
            bboxes_in = bboxes_in.permute(0, 2, 1).contiguous().to(torch.float32)
            scores_in = scores_in.permute(0, 2, 1).contiguous().to(torch.float32)
            ...
            bboxes, probs = parallel_scale_back_batch(
                bboxes_in, scores_in, self.dboxes_xywh, self.scale_xy, self.scale_wh
            )
            ...
            for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):
                bbox = bbox.squeeze(0)
                prob = prob.squeeze(0)
                bboxes_out, labels_out, scores_out = self.decode_single(
                    bbox, prob, criteria, max_output
                )
                output_bboxes_.append(bboxes_out)
                output_labels_.append(labels_out)
                output_scores_.append(scores_out)
                output_length_.append(scores_out.size(0))
            return (
                torch.cat(output_bboxes_),
                torch.cat(output_labels_),
                torch.cat(output_scores_),
                torch.from_numpy(np.array(output_length_)),
            )

    # perform non-maximum suppression for IPEX tensor
    def decode_single_ipex(
        ...
        bboxes_out, labels_out, scores_out = batch_score_nms(
            bboxes_in, scores_in, criteria
        )
