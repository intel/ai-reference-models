...
def parse_args():
    ...
    parser.add_argument("--device", "-did", type=int, help="device id")
    parser.add_argument(
        "-j",
        "--workers",
        default=4,
    ...
    parser.add_argument(
        "--autocast", action="store_true", default=False, help="enable autocast"
    )
    parser.add_argument(
        "--profile", action="store_true", default=False, help="enable profile"
    )
    parser.add_argument(
        "--number-instance",
        default=7,
    ...
    parser.add_argument(
        "--start-core", default=-1, type=int, help="the start core to creat cpu pool"
    )
    parser.add_argument(
        "--end-core", default=-1, type=int, help="the end core to creat cpu pool"
    )
    parser.add_argument(
        "--accuracy-mode",
        action="store_true",
...
def coco_eval(model, val_dataloader, cocoGt, encoder, inv_map, args):
    ...
    if args.accuracy_mode:
        ...
        if args.iteration is None:
            ...
            exit(-1)
        epoch_number = (
            (args.iteration // len(val_dataloader) + 1)
    ...
    if args.int8:
        ...
        with torch.no_grad():
            model_decode.model.model = optimization.fuse(
                model_decode.model.model, inplace=False
            )
            qconfig = QConfig(
            ...
            example_inputs = torch.randn(
                (
                    (args.batch_size // args.number_instance)
            ...
            if args.calibration:
                print(
                    "runing int8 LLGA calibration step not support in throughput benchmark"
                )
            else:
                ...
                with torch.no_grad():
                    model_decode = torch.jit.trace(
                        model_decode, example_inputs, check_trace=False
                    ).eval()
                ...
                if args.use_throughput_benchmark:
                    ...
                    print(
                        "runing int8 real inputs inference use_throughput_benchmark path"
                    )
                    bench = ThroughputBenchmark(model_decode)
                    if args.iteration > len(val_dataloader):
                        print(
                            "Using ThroughputBenchmark, but args.iteration:{0} larger than len(val_dataloader):{1}".format(
                                args.iteration, len(val_dataloader)
                            )
                        )
                        exit(-1)
                    for nbatch, (img, img_id, img_size, bbox, label) in enumerate(
                        val_dataloader
                    ):
                    ...
                    if args.profile:
                        ...
                        with torch.profiler.profile(
                            ...
                            stats = bench.benchmark(
                                num_calling_threads=args.number_instance,
                                num_warmup_iters=args.warmup_iterations,
                                num_iters=args.iteration,
                            )  # num_instance, warm up iters, total iters
                    ...
                    print(
                        "runing int8 real inputs inference use_multi_stream_module path. async_execution is:{}".format(
                            args.async_execution
                        )
                    )
                    if args.async_execution:
                        if args.start_core != -1 and args.end_core != -1:
                            cpu_pool = ipex.cpu.runtime.CPUPool(
                                core_ids=range(args.start_core, args.end_core + 1)
                            )
                        elif args.instance_number != -1:
                            cpu_pool = ipex.cpu.runtime.CPUPool(
                                node_id=args.instance_number
                            )
                        else:
                            print(
                                "args.instance_number or (args.start_core, args.end_core) must be indicated to create cpu_pool"
                            )
                            exit(-1)
                        model_decode = ipex.cpu.runtime.MultiStreamModule(
                            model_decode,
                            num_streams=args.number_instance,
                            cpu_pool=cpu_pool,
                            concat_output=False,
                        )
                    time_consume = 0
                    ...
                    for _ in range(epoch_number):
                        for nbatch, (img, img_id, img_size, bbox, label) in enumerate(
                            ...
                            if total_iteration >= args.warmup_iterations:
                                ...
                                print(
                                    "time of step {0} is: {1}".format(
                                        total_iteration, step_time
                                    )
                                )
                            ...
                            if args.accuracy_mode:
                                ...
                                idx = 0
                                if args.async_execution:
                                    mini_batch_of_each_stream = get_bs_per_stream(
                                        img.size(0), args.number_instance
                                    )
                                    used_num_streams = (
                                        mini_batch_of_each_stream.__len__()
                                    )
                                    for i in range(used_num_streams):
                                        idx2 = 0
                                        mini_bs_of_this_stream = (
                                            mini_batch_of_each_stream[i]
                                        )
                            ...
                            if total_iteration == args.iteration:
                                fps = (
                                    args.batch_size
                                    * (args.iteration - args.warmup_iterations + 1)
                                    / time_consume
                                )
                                avg_time = (
                                    time_consume
                                ...
                                print(
                                    ...
                                    % (args.instance_number, avg_time, fps)
                                )
                                break
                            total_iteration += 1
                else:
                    print(
                        "runing int8 real inputs inference pthread weight sharing path"
                    )
                            ...
                            for _ in range(epoch_number):
                                for nbatch, (
                                    ...
                                    if total_iteration == args.iteration:
                                        ...
                                        p99 = np.percentile(timeBuff, 99)
                                        print(
                                            "P99 Latency {:.2f} ms".format(p99 * 1000)
                                        )
                                        print(
                                            "Instance num: %d Avg Time/Iteration: %f msec Throughput: %f fps"
                                            % (tid, avg_time, fps)
                    ...
                    for i in range(1, args.number_instance + 1):
                        thread = threading.Thread(
                            target=run_model, args=(model_decode, i)
                        )
        ...
        if args.dummy:
            ...
            if args.autocast:
                ...
                model_decode.model = model_decode.model.to(
                    memory_format=torch.channels_last
                )
                if use_ipex:
                    print("bf16 block format weights cache enabled")
                    # model.model = ipex.optimize(model.model, dtype=torch.bfloat16, level='O0')
                    model_decode.model.model = ipex.optimize(
                        model_decode.model.model, dtype=torch.bfloat16, inplace=False
                    )
                ...
                if args.jit:
                    ...
                    with (
                        ...
                        model_decode = torch.jit.trace(
                            model_decode,
                            torch.randn(batch_per_stream, 3, 1200, 1200).to(
                                memory_format=torch.channels_last
                            ),
                        ).eval()
                    ...
                    if use_ipex:
                        print(
                            "Ipex Autocast imperative path in throughput benchmark not support"
                        )
                    else:
                        print(
                            "OOB Autocast imperative path in throughput benchmark not support"
                        )
                        exit(-1)
            else:
                if args.bf32:
                ...
                if use_ipex:
                    ...
                    model_decode.model.model = ipex.optimize(
                        model_decode.model.model, dtype=torch.float32, inplace=False
                    )
                if args.jit:
                    print("enable jit")
                    with torch.no_grad():
...
def eval_ssd_r34_mlperf_coco(args):
    ...
    val_trans = SSDTransformer(
        dboxes, (args.image_size[0], args.image_size[1]), val=True
    )
...
def main():
    ...
    if not args.no_cuda:
        ...
        if not "USE_IPEX" in os.environ:
            print(
                'Set environment variable "USE_IPEX" to 1 (export USE_IPEX=1) to utilize Intel® Extension for PyTorch, if needed.'
            )
