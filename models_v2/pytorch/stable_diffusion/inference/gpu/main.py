...
parser.add_argument(
    "--prompt", default="nateraw/parti-prompts", type=str, help="prompt_dataset"
)
parser.add_argument("--batch_size", default=1, type=int, help="batch size")
parser.add_argument(
    "--idx_start", default=0, type=int, help="select the start index of image"
)
parser.add_argument(
    "--precision",
...
parser.add_argument(
    "--save_image", action="store_true", default=False, help="save image"
)
parser.add_argument(
    "--save_tensor", action="store_true", default=False, help="save tensor"
)
parser.add_argument(
    "--accuracy",
    action="store_true",
    default=False,
    help="compare the result with cuda",
)
parser.add_argument(
    "-m",
    "--model_id",
...
parser.add_argument(
    "--save_path", default="./xpu_result", type=str, help="output image dir"
)
parser.add_argument(
    "--num_inference_steps", default=50, type=int, help="number of unet step"
)
...
parser.add_argument(
    ...
    default="fid",
    type=str,
    help="evaluation method, now we suppor clip and fid",
)
parser.add_argument(
    "--pipeline_mode",
...
parser.add_argument(
    "--channels_last", action="store_true", default=False, help="enable channels_last"
)
...
def main():
    ...
    if args.pipeline_mode == "img2img":
        pipe = StableDiffusionImg2ImgPipeline.from_pretrained(
            args.model_id, torch_dtype=datatype
        )
    else:
    ...
    if args.device == "xpu":
        ...
        pipe.text_encoder = torch.xpu.optimize(
            pipe.text_encoder.eval(), dtype=datatype, inplace=True
        )

        if not args.disable_optimize_transformers and args.precision == "fp16":
            # optimize with ipex
            pipe.unet = ipex.optimize_transformers(
                pipe.unet.eval(), dtype=datatype, device=args.device, inplace=True
            )
            ...
            if args.device == "xpu":
                ...
                if profiling:
                    torch.save(
                        ...
                        "stale_diffusion_inf_profile.pt",
                    )
                    ...
                    prof.export_chrome_trace(
                        "./stable_diffusion_inf_profile_trace.json"
                    )
            elif args.device == "cuda":
                start_time = time.time()
                images = pipe(
            ...
            if args.evaluate_method == "clip":
                ...
                if args.accuracy:
                    for i in range(args.batch_size):
                        name = (
                            "result_{}_{}.png".format(idx1 + i, iter)
                            if args.save_image
                            else "result_{}_{}.pt".format(idx1 + i, iter)
                        )
