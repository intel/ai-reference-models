...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_path", type=str, default=None, help="COCO2017 dataset path"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="fp32",
        help="precision: fp32, bf32, bf16, fp16, int8-bf16, int8-fp32",
    )
    parser.add_argument(
        "--calibration",
        action="store_true",
        default=False,
        help="doing calibration step for LCM int8",
    )
    parser.add_argument(
        "--configure-dir",
        ...
        type=str,
        metavar="PATH",
        help="path to LCM int8 configures, default file name is configure.json",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="compile with ipex backend",
    )
    parser.add_argument(
        "--compile_inductor",
        action="store_true",
    ...
    parser.add_argument(
        "--benchmark", action="store_true", default=False, help="test performance"
    )
    parser.add_argument(
...
def run_weights_sharing_model(pipe, tid, args):
    ...
    for i in range(args.iterations + args.warmup_iterations):
        ...
        if (
            ...
            or args.precision == "int8-bf16"
        ):
            with torch.autocast("cpu", dtype=args.dtype), torch.no_grad():
                output = pipe(
                    args.prompt, generator=torch.manual_seed(args.seed)
                ).images
        else:
            with torch.no_grad():
                output = pipe(
...
def main():
    ...
    if args.precision == "fp32":
        ...
        raise ValueError(
            "--precision needs to be the following: fp32, bf32, bf16, fp16, int8-bf16, int8-fp32"
        )

    if args.compile_inductor:
        pipe.precision = torch.float32
    elif (
        args.model_name_or_path == "SimianLuo/LCM_Dreamshaper_v7"
        and args.precision == "int8-bf16"
    ):
        pipe.precision = torch.float32
    else:
    ...
    if args.model_name_or_path == "stabilityai/stable-diffusion-2-1":
        ...
        input = (
            torch.randn(2, 4, 96, 96)
            .to(memory_format=torch.channels_last)
            .to(dtype=pipe.precision),
            torch.tensor(921),
            torch.randn(2, 77, 1024).to(dtype=pipe.precision),
        )
        ...
        raise ValueError(
            "This script currently only supports stabilityai/stable-diffusion-2-1 and SimianLuo/LCM_Dreamshaper_v7."
        )
    ...
    if args.ipex:
        ...
        if args.precision == "fp32":
            pipe.text_encoder = ipex.optimize(
                pipe.text_encoder.eval(), inplace=True, sample_input=text_encoder_input
            )
            ...
            pipe.text_encoder = ipex.optimize(
                ...
                sample_input=text_encoder_input,
            )
            pipe.unet = ipex.optimize(
                pipe.unet.eval(), inplace=True, auto_kernel_selection=True
            )
            pipe.vae = ipex.optimize(
                pipe.vae.eval(), inplace=True, auto_kernel_selection=True
            )
            ...
            if args.model_name_or_path == "stabilityai/stable-diffusion-2-1":
                ...
                pipe.vae = ipex.optimize(
                    pipe.vae.eval(), dtype=args.dtype, inplace=True
                )
            else:
                if not args.calibration:
                    # pipe.text_encoder = ipex.optimize(pipe.text_encoder.eval(), dtype=args.dtype, inplace=True, sample_input=text_encoder_input)
                    qconfig_mapping = ipex.quantization.default_static_qconfig_mapping
                    pipe.unet = ipex.quantization.prepare(
                        pipe.unet.eval(), qconfig_mapping, input, inplace=True
                    )
    ...
    val_coco = dset.CocoCaptions(
        root="{}/val2017".format(args.dataset_path),
    ...
    if args.distributed:
        val_sampler = torch.utils.data.distributed.DistributedSampler(
            val_coco, shuffle=False
        )
    else:
        val_sampler = None

    val_dataloader = torch.utils.data.DataLoader(
        val_coco, batch_size=1, shuffle=False, num_workers=0, sampler=val_sampler
    )

    if (
        args.model_name_or_path == "SimianLuo/LCM_Dreamshaper_v7"
        and args.ipex
        and args.calibration
    ):
        print("runing LCM int8 calibration step\n")
        qconfig_mapping = ipex.quantization.default_static_qconfig_mapping
        pipe.unet = ipex.quantization.prepare(
            pipe.unet, qconfig_mapping, input, inplace=True
        )
    ...
    if args.compile_ipex:
        ...
        if args.precision == "fp32" or args.precision == "bf16":
            ...
            pipe.text_encoder = ipex.optimize(
                pipe.text_encoder.eval(), dtype=args.dtype, weights_prepack=False
            )
            pipe.unet = ipex.optimize(
                pipe.unet.eval(), dtype=args.dtype, weights_prepack=False
            )
            pipe.vae = ipex.optimize(
                pipe.vae.eval(), dtype=args.dtype, weights_prepack=False
            )
            ...
            raise ValueError(
                "If you want to use torch.compile with ipex backend, --precision needs to be the following: fp32, bf16"
            )
    ...
    if args.compile_inductor:
        ...
        if args.precision == "fp32":
            ...
            raise ValueError(
                "If you want to use torch.compile with inductor backend, --precision needs to be the following: fp32, bf16, int8-bf16, int8-fp32"
            )
        ...
        if args.weight_sharing:
            ...
            for i in range(1, args.number_instance + 1):
                thread = threading.Thread(
                    target=run_weights_sharing_model, args=(pipe, i, args)
                )
    ...
    if args.profile:
        ...
        timeline_file = (
            timeline_dir
            + "timeline-"
            + str(torch.backends.quantized.engine)
            + "-"
            + "stable_diffusion-"
            + "-"
            + str(os.getpid())
