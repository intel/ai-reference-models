...
def test(
        ...
        if ipex:
            ...
            if int8:
                ...
                with torch.no_grad():
                    ...
                    model = torch.compile(converted_model, backend="ipex", dynamic=rect)
                ...
                if jit:
                    if bf16:
                        model = TracedModel(
                            model=model, dtype=torch.bfloat16, sample_input=x
                        )
                    elif fp16:
                        model = TracedModel(
                            model=model, dtype=torch.float16, sample_input=x
                        )
                    else:
            ...
            if int8:
                ...
                with torch.no_grad():
                    ...
                    if ipex:
                        print("[Info] Running torch.compile() with IPEX backend")
                        model = torch.compile(
                            converted_model, backend="ipex", dynamic=rect
                        )
                    else:
    ...
    if performance:
        ...
        if opt.warmup_iterations >= opt.number_iter:
            print(
                f"WARNING: warmup_iter {opt.warmup_iterations} is larger than number_iter {opt.number_iter}, Invalid setup. Please reset your warmup_iter and number_iter!"
            )
                ...
                for epoch in range(opt.inf_epoch_number):
                    for i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader)):
                        ...
                        (
                            ...
                            height,
                            width,
                        ) = img.shape  # batch size, channels, height, width
