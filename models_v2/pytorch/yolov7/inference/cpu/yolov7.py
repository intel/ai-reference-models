        ...
        for epoch in range(opt.inf_epoch_number):
            for batch_i, (img, _, _, shapes) in enumerate(dataloader):
                if epoch * len(dataloader) + batch_i >= opt.number_iter:
                    break
                img = img.float().contiguous(memory_format=torch.channels_last)  # uint8 to fp12
                ...
                if not opt.jit and opt.bf16:
                    ...
                    with torch.amp.autocast('cpu', dtype=torch.half):
                        out = model(img, augment=augment)[0]
                else:
                    out = model(img, augment=augment)[0]
                end_time = time.time()

                out = non_max_suppression(out.float(), conf_thres=opt.conf_thres, iou_thres=opt.iou_thres, multi_label=True)
...
def test(opt,
    ...
    if weight_sharing:
        assert performance and batch_size, \
    ...
    val = data.get("val")
    data["val"] = [os.path.join(opt.dataset_dir, x) for x in (val if isinstance(val, list) else [val])]
    ...
    dataloader = create_dataloader(data[task], imgsz, batch_size, gs, opt, pad=0.5, rect=rect,
    ...
    if evaluate:
        ...
        if ipex:
            ...
            if int8:
                ...
                if jit:
                    if bf16:
                        model = TracedModel(model=model, dtype=torch.bfloat16, sample_input=x)
                    elif fp16:
                        model = TracedModel(model=model, dtype=torch.float16, sample_input=x)
                    else:
        ...
        # torch.compile() inductor path
            ...
            if int8:
                ...
                with torch.no_grad(), torch.amp.autocast('cpu', dtype=torch.bfloat16):
                    ...
                    if ipex:
                        ...
                        model = torch.compile(model, backend="ipex", dynamic=rect)
                    else:
                        print('[Info] Running torch.compile() BFloat16 with default backend')
                        model = torch.compile(model, dynamic=rect)
            elif fp16:
                with torch.no_grad(), torch.amp.autocast('cpu', dtype=torch.half):
                    x = x.to(torch.half)
                    if ipex:
                        print('[Info] Running torch.compile() FPloat16 with IPEX backend')
                        model = torch.compile(model, backend="ipex", dynamic=rect)
                    else:
                        print('[Info] Running torch.compile() FPloat16 with default backend')
                        model = torch.compile(model, dynamic=rect)
            else:
                with torch.no_grad():
                    if ipex:
            ...
            with torch.no_grad(), torch.amp.autocast('cpu', enabled=bf16 or fp16, dtype=torch.half if fp16 else torch.bfloat16):
                _ = model(x, augment)[0]
                _ = model(x, augment)[0]
        ...
        with torch.no_grad():
            if dataloader:
                for batch_i, (img, targets, paths, shapes) in enumerate(dataloader):
                    ...
                    img /= 255.0  # 0 - 255 to 0.0 - 1.0
                    prepared_model(img, augment)
                    if batch_i == opt.calibration_steps:
                        break
            else:
                for _ in range(opt.calibration_steps):
                    x = torch.rand(batch_size, 3, imgsz, imgsz).contiguous(memory_format=torch.channels_last)
                    prepared_model(x, augment)
            prepared_model.save_qconf_summary(configure_dir)
    ...
    if performance:
        ...
        if opt.warmup_iterations >= opt.number_iter:
            print(f"WARNING: warmup_iter {opt.warmup_iterations} is larger than number_iter {opt.number_iter}, Invalid setup. Please reset your warmup_iter and number_iter!")
        opt.inf_epoch_number = (opt.number_iter // len(dataloader) + 1) if (opt.number_iter > len(dataloader)) else 1
            ...
            if weight_sharing:
                ...
                for epoch in range(opt.inf_epoch_number):
                    for i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader)):
                        ...
                        out = non_max_suppression(out.float(), conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
        ...
        if save_json and len(jdict):
            ...
            anno_json = os.path.join(opt.dataset_dir, 'coco/annotations/instances_val2017.json')  # annotations json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ipex', action='store_true', default=False, help='use intel pytorch extension')
    parser.add_argument('--inductor', action='store_true', default=False, help='use torch.compile() default inductor backend')
    parser.add_argument('--seed', default=None, type=int, help='seed for initializing inference. ')
    parser.add_argument('--int8', action='store_true', default=False, help='enable ipex int8 path')
    ...
    parser.add_argument('--fp16', action='store_true', default=False, help='enable ipex fp16 path')
    parser.add_argument('--jit', action='store_true', default=False, help='enable ipex jit fusionpath')
    ...
    parser.add_argument("--performance", action='store_true', help="test the performance of inference")
    parser.add_argument('-w', '--warmup-iterations', default=20, type=int, metavar='N', help='number of warmup iterati ons to run')
    ...
    parser.add_argument('--dataset-dir', default="", type=str, help='directory which contains coco dataset')
    parser.add_argument('--checkpoint-dir', default="", type=str, help='directory which contains pretrained model')
    parser.add_argument('--calibration-steps', default=10, type=int, help='calibration iteration number')
    parser.add_argument('--drop-last', action='store_true', default=False, help="drop last iteration for val dataloader")
