...
def create_eval_dataset(args, worker_init_fn):
    ...
    eval_dataloader = DataLoader(
        eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=0, pin_memory=True
    )
...
def parse_args():
    ...
    parser.add_argument("--max_steps_for_scheduler",
                        default=218880,
                        type=int,
    ...
    parser.add_argument(
        "--converge", default=False, action="store_true", help="Whether to run convergence."
    )
    parser.add_argument(
        "--dist_lamb",
        default=False,
    ...
    parser.add_argument('--disable-broadcast-buffers', action='store_true',
...
def setup_training(args):
    ...
    if (int(os.environ.get("PMI_SIZE", "0")) > 1 or args.world_size > 1) and not args.multi_instance:
        ...
        if args.world_size > 1:
            ...
            print('world_size:{}, rank:{}, device:{}'.format(
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.resume_from_checkpoint and args.resume_checkpoint is not None:
        if not os.path.exists(args.resume_checkpoint):
            raise RuntimeError("The checkpoint ",
                               args.resume_checkpoint, " doesn't exist")
        print('load...', args.resume_checkpoint, ' to ',
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any(
            nd in n for nd in no_decay)], 'weight_decay': args.weight_decay_rate},
...
def run_eval(
    ...
    with torch.no_grad():
        for step, batch in enumerate(eval_dataloader):
            ...
            if args.converge:
                print('Eval [', step, '/', len(eval_dataloader), '] loss: ', round(float(total_eval_loss / total_masked), 4), ' acc: ',
                      mlm_acc.item(), '(avg acc: ', round(float(total_eval_mlm_acc.item() / total_masked), 4), ' num masked: ', num_masked, ')')

    model.train()
    total_masked = torch.tensor(total_masked, device=device, dtype=torch.int64)
    total_eval_loss = torch.tensor(
        total_eval_loss, device=device, dtype=torch.float64)
    if torch.distributed.is_initialized():
...
def calc_accuracy(outputs, masked_lm_labels, next_sentence_label, args):
    ...
    mlm_acc, num_masked = calc_mlm_acc(
        outputs, masked_lm_labels, args.dense_seq_output)
    seq_acc_t = torch.argmax(seq_relationship_logits, dim=-
                             1).eq(next_sentence_label.view([-1])).to(torch.float)
...
def main():
    ...
    if args.converge:
        print('For convergence verbose show freq, PRINT_ITER_FACTOR = ',
              PRINT_ITER_FACTOR)
    status = 'aborted'  # later set to 'success' if termination criteria met
    device, args = setup_training(args)
    print("args.world_size={}, args.rank={}".format(
    ...
    if args.do_train:
        ...
        if args.converge:
            ...
            print('all eval steps, start: ', eval_steps[0], ', end: ', eval_steps[len(eval_steps) - 1], ', eval times: ', len(eval_steps))
        ...
        if found_resume_checkpoint(args) and checkpoint is not None:
            ...
            random.Random(shuffling_seeds[epoch %
                                          len(shuffling_seeds)]).shuffle(files)
            f_start_id = 0
    ...
    while global_step < args.max_steps and not end_training:
        ...
        if torch.distributed.is_initialized() and args.world_size > num_files:
            ...
            data_file = files[(f_start_id*args.world_size + args.rank +
                               remainder * f_start_id) % num_files]
            print('rank: ', args.rank, '. data file: ', data_file)
        else:
            data_file = files[(f_start_id*args.world_size +
                               args.rank) % num_files]
        ...
        if torch.distributed.is_initialized() and not args.no_ddp:
            ...
            train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=args.workers, pin_memory=True)

        for f_id in range(f_start_id, len(files)):

            # data file for next file loop train
            if args.world_size > num_files:
                data_file = files[(f_id*args.world_size + args.rank +
                                   remainder * f_id) % num_files]
            else:
                data_file = files[(f_id*args.world_size +
                                   args.rank) % num_files]
