...
class DistilBertForMaskedLM_Trace(DistilBertForMaskedLM):
    ...
    def forward(
        ...
        return (
            prediction_logits,
            dlbrt_output.hidden_states,
            dlbrt_output.attentions,
            mlm_loss,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="cpu",
        help="The running device (cpu, cuda, xpu).",
    )
    ...
    parser.add_argument(
        "--student_name", type=str, required=True, help="The student model."
    )
    ...
    parser.add_argument(
        "--batch_size", type=int, default=64, help="Batch size (for each process)."
    )
    parser.add_argument(
        "--num_iteration", type=int, default=-1, help="Iteration number."
    )
    parser.add_argument(
        "--group_by_size",
        action="store_false",
    ...
    if args.student_pretrained_weights is not None:
        ...
        student = student_model_class.from_pretrained(
            args.student_name, config=stu_architecture_config
        )
