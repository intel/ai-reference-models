...
from utils import logger, MAP_TORCH_DTYPE
...
class Distiller:
    def __init__(
        ...
        if params.run_mode == "train":
            with open(params.data_file, "rb") as fp:
                data = pickle.load(fp)
    ...
    def round_batch(self, x: torch.tensor, lengths: torch.tensor):
        ...
        """
        if not self.dtype == "FP16" or len(lengths) < 8:
            return x, lengths
    ...
    def step(
        ...
        self.n_sequences_epoch += input_ids_.size(0)
    ...
    def eval(self):
        files = [
            os.path.join(self.params.data_file, f)
            for f in os.listdir(self.params.data_file)
            if os.path.isfile(os.path.join(self.params.data_file, f)) and "part" in f
        ]
        files.sort()
        num_files = len(files)
