...
if [[ "${PLATFORM}" == "Max" ]]; then
    ...
    if [[ "${PRECISION}" != fp16 ]]; then
      echo "QuartzNet inference currently supports only fp16 precision"
      exit 1
    fi
fi
...
echo "quartznet ${PRECISION} inference plain MultiTile=${MULTI_TILE} BS=${BATCH_SIZE}"
...
modelname=quartz_net

if [[ ${MULTI_TILE} == "False" ]]; then
    rm ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
    python inference.py --dataset_dir ${DATASET_DIR} --val_manifest ${DATASET_DIR}/librispeech-test-other-wav.json --model_config configs/quartznet15x5_speedp-online-1.15_speca.yaml --output_dir ${OUTPUT_DIR}/results  --batch_size ${BATCH_SIZE} --warmup_steps 5 --amp --ckpt ${CKPT_DIR}/nvidia_quartznet_210504.pt --save_prediction ${OUTPUT_DIR}/results/test-other.predictions --torchscript --torchscript_export --dynamic_shape 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
