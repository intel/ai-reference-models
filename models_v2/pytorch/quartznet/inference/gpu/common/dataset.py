...
class AudioDataset(Dataset):
    def __init__(self, data_dir, manifest_fpaths, labels,
                 ...
                 shift_perturbation=None, ignore_offline_speed_perturbation=False, dynamic_shape=False):
        ...
        ## use fix shape if args.dynamic_shape = False
        if dynamic_shape == False:
            dataset_length = len(self.samples)
            for item in self.samples:
                if item['audio_filepath'][0].split("test-other-wav/")[1] == '3764/168671/3764-168671-0005.wav':
                    sample = item
            self.samples = [sample]*dataset_length
...
def get_data_loader(dataset, batch_size, multi_gpu=True, shuffle=True,
    ...
    kw = {'dataset': dataset, 'collate_fn': collate_fn,
          'num_workers': num_workers, 'pin_memory': True, 'pin_memory_device': 'xpu'}
