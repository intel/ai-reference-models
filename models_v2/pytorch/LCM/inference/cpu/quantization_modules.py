...
class QuantizedLinear(Linear):
    def forward(self, x):
        return super().forward(self.input_quant(x.float())).dequantize().bfloat16()
...
class FakeQuantLinear(torch.nn.Linear):
    def __init__(self, module: torch.nn.Linear):
        ...
        self.add_module(
            "activation_pre_process", default_fake_quant(reduce_range=ACT_REDUCE_RANGE)
        )
        self.add_module(
            "weight_fake_quant",
            default_per_channel_weight_fake_quant(reduce_range=WEIGHT_REDUCE_RANGE),
        )
        self.add_module(
            "activation_post_process", default_fake_quant(reduce_range=ACT_REDUCE_RANGE)
        )
        self.is_lora_layer = (
            True if peft_available and isinstance(module, LoRALinear) else False
        )
    ...
    def forward(self, x):
        ...
        if (
            ...
            and self.r[self.active_adapter] > 0
        ):
            lora_weight = (
                transpose(
...
def convert_to_fp32model(model):
    ...
    for key in key_list:
        ...
        if isinstance(target, QuantizedLinear):
            setattr(parent, target_name, FP32Linear(target))
        elif isinstance(target, QuantizedConv2d):
            setattr(parent, target_name, FP32Conv2d(target))
    return model
...
def sync_all_observers(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, FakeQuantize):
            ...
            _scale, _zero_point = _scale.to(module.scale.device), _zero_point.to(
                module.zero_point.device
            )
