...
PROFILE=${PROFILE:-profiles/c100.bf16.csv}
...
if [[ ${PLATFORM} == "Flex" || ${PLATFORM} == "Max" ]]; then
  IMAGE=${IMAGE:-intel/image-recognition:pytorch-flex-gpu-fbnet-inference}
  platform_opt="--device /dev/dri/"
elif [[ ${PLATFORM} == "CUDA" ]]; then
  IMAGE=${IMAGE:-intel/image-recognition:pytorch-cuda-gpu-fbnet-inference}
