...
def parse_args(parser):
    ...
    parser.add_argument('--torchjit', action='store_true',
                        help='Run inference with TorchJit model (convert to TS if needed)')
...
def prepare_input_sequence(fields, device, symbol_set, text_cleaners,
                           batch_size=8, dataset=None, load_mels=False,
...
class MeasureTime(list):
    def __init__(self, *args, cuda=False, xpu=True, **kwargs):
    ...
    def __add__(self, other):
        ...
        if self.cuda:
            ...
            return MeasureTime((sum(ab) for ab in zip(self, other)), xpu=self.xpu)
...
def main():
    ...
    batches = prepare_input_sequence(
        fields, cpu_device, args.symbol_set, args.text_cleaners, args.batch_size,
    ...
    if vocoder is not None and device == torch.device('xpu'):
        vocoder.eval()
    ...
    gen_measures = MeasureTime(cuda=args.cuda, xpu=args.xpu)
    vocoder_measures = MeasureTime(cuda=args.cuda, xpu=args.xpu)
    ...
    profiling_enabled = os.environ.get("PROFILE", "OFF").upper() in ["1", "Y", "ON", "YES", "TRUE"]
    ...
    for rep in (tqdm(range(reps), 'Inference') if reps > 1 else range(reps)):
        ...
        for b in batches:
            ...
            # save result 1
            if args.report_mel_loss:
                gen_mel_loss_sum += compute_mel_loss(mel.cpu(), mel_lens.cpu(), b['mel'].cpu(), b['mel_lens'].cpu())
            ...
            if args.report_mel_loss:
                voc_mel_loss_sum += mel_loss_fn(audios, mel.cpu(), mel_lens.cpu())
            ...
            if args.output is not None:
                for i, audio in enumerate(audios):
                    ...
                    print("generatedi:", fname)
                    audio_path = Path(args.output, fname)
                    write(audio_path, args.sampling_rate, audio.numpy())
            ...
            if profiling_enabled:
                write_profile_info(prof, args.batch_size)

            all_utterances += mel.size(0)
            all_samples += mel_lens.sum().item() * args.hop_length
            all_batches += 1

    print_perf_info(total_time, counter, args.batch_size)
