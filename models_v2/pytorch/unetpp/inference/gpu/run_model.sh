...
if [[ "${PLATFORM}" == "Max" ]]; then
    echo "only support Flex platform"
    exit 1
elif [[ "${PLATFORM}" == "Flex" ]]; then
    BATCH_SIZE=${BATCH_SIZE:-8}
    PRECISION=${PRECISION:-fp16}
    if [[ "${PRECISION}" != fp16 ]]; then
      echo "Unetpp model currently supports only fp16 precision"
      exit 1
    fi
fi
...
echo "UNetpp ${PRECISION} inference plain MultiTile=${MULTI_TILE} BS=${BATCH_SIZE}"
...
if [[ ${MULTI_TILE} == "False" ]]; then
    ...
    bash cmd_infer.sh ${BATCH_SIZE} ${PRECISION} 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
