...
def init_distributed(rank=-1, local_rank=-1, size=-1, use_device="cpu", backend=""):
    ...
    if size > 1:
        ...
        if hasattr(dist, "all_to_all_single"):
            try:
                a = torch.rand([1, 3]).xpu("xpu:{}".format(my_local_rank))
                dist.broadcast(a, src=0)
                print("Successfully execute distributed broadcast on the xpu:{}".format(this_rank))

                tensor_list = [torch.zeros(2, dtype=torch.int64).xpu("xpu:{}".format(my_local_rank)) for _ in range(2)]
                tensor = (torch.arange(2, dtype=torch.int64) + 1 + 2 * rank).xpu("xpu:{}".format(my_local_rank))
