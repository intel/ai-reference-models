...
def dlrm_wrap(X, lS_o, lS_i, use_gpu, use_xpu, device, args, amp, amp_dtype, ndevices=1):
...
def dlrm_wrap_dynamo(X, lS_o, lS_i, use_gpu, use_xpu, device, args, amp, amp_dtype, ndevices=1):
    with record_function("DLRM forward"):
        if ndevices == 1:
            lS_i = (
                [S_i.to(device) for S_i in lS_i]
...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        for i in range(0, ln.size):
            ...
            if self.qr_flag and n > self.qr_threshold:
                ...
                _m = m[i] if self.md_flag else m
                EE = nn.EmbeddingBag(n, _m, mode="sum", sparse=True)
    ...
    def __init__(
        ...
        if (
            ...
            if ext_dist.my_size > 1:
                ...
                print("self.n_global_emb:", self.n_global_emb)
                print("self.n_local_emb:", self.n_local_emb)
                print("self.n_emb_per_rank:", self.n_emb_per_rank)
                print("self.local_emb_slice:", self.local_emb_slice)
                print("self.local_emb_indices:", self.local_emb_indices)
    ...
    def interact_features(self, x, ly):
        ...
        if args.fp16:
            ly = [t.half() if t.dtype != torch.float16 else t for t in ly]
            x = x.half() if x.dtype != torch.float16 else x
        elif args.bf16:
            ly = [t.bfloat16() if t.dtype != torch.bfloat16 else t for t in ly]
    ...
    def parallel_forward(self, dense_x, lS_o, lS_i):
        ...
        if not (self.use_device=="xpu"):
            device_ids = range(ndevices)
        else:
            device_ids = []
            for i in range(ndevices):
                device_ids.append(torch.device("xpu", i))
...
def inference(
            ...
            for i, testBatch in enumerate(test_ld):
                ...
                if i == 0:
                    ...
                    if ext_dist.my_size <= 1:
                        if ndevices == 1:
                            lS_i = (
                                [S_i.to(device) for S_i in lS_o_test]
                ...
                with (torch.autograd.profiler_legacy.profile(profile, use_xpu=True) if use_xpu else torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA])) as prof1:
                    # forward pass

                    if use_xpu and (not (ext_dist.my_size > 1)):
                        try:
                            import memory_check
                ...
                if profile and i == 15:
                    if use_xpu:
                        rank_index = ""
                        if ext_dist.my_size > 1:
                            rank_index = "rank_" + str(ext_dist.my_local_rank) + "_"
                        path ='./report/dlrm_inference_' + rank_index + 'profiling'
                        torch.save(prof1.key_averages().table(sort_by="self_xpu_time_total", row_limit=100000), \
                            path + '.pt')
                        prof1.export_chrome_trace(path + '.json')
                        torch.save(prof1.table(sort_by="id", row_limit=100000), path + '_detail.pt')
                    else:
                        print(prof.key_averages().table(sort_by=sort_key, row_limit=100000))
    else:
        for i, testBatch in enumerate(test_ld):
            # early exit if nbatches was set by the user and was exceeded
            if nbatches > 0 and i >= nbatches:
    ...
    if inference_only:
        if total_time == 0:
            print("The total_time is zero, please check args.")
        else:
            print('dlrm_inf latency: ', total_time, ' s')
            print('dlrm_inf avg time: ', total_time/time_count, ' s, ant the time count is :', time_count)
            print('dlrm_inf throughput: ', batch * time_count/total_time, ' samples/s')
...
def run():
    ...
    parser.add_argument("--dynamo", action="store_true", default=False)
    ...
    use_xpu = args.use_xpu and torch.xpu.is_available()
    ...
    if not args.debug_mode:
        ext_dist.init_distributed(
            local_rank=args.local_rank, use_device=use_device, backend=args.dist_backend
        )
    ...
    if use_gpu:
        ...
        if ext_dist.my_size > 1:
            ngpus = 1
            device = torch.device("xpu", ext_dist.my_local_rank)
        else:
            if args.parallel:
    ...
    if args.mlperf_logging:
        mlperf_logger.barrier(use_device)
    ...
    if args.data_generation == "dataset":
        ...
        ln_emb = np.array([2500000, 39043, 17289, 7420, 20263, 3, 7120, 1543, 63, 2500000, 2500000, 403346, 10, 2208, 11938, 155, 4, 976, 14, 2500000, 2500000, 2500000, 585935, 12972, 108, 36])
        m_den = ln_bot[0]
    ...
    if args.md_flag:
        ...
        for i in range(len(ln_emb)):
            if ln_emb[i] <= args.md_threshold:
                _m_spa[i] = 128
        m_spa = map(int, _m_spa)
        m_spa = list(m_spa)
    ...
    global ndevices
    ndevices = min(ngpus, args.mini_batch_size, num_fea - 1) if use_gpu or use_xpu else -1
    ...
    if use_gpu:
        ...
        #    dlrm = dlrm.to(device_ids)  # .xpu()
        #else:
        dlrm = dlrm.to(device)  # .xpu()
    ...
    if not (args.load_model == ""):
        ...
        if use_gpu:
            ...
            if dlrm.ndevices > 1:
                ...
                # to multiple GPUs to be done in parallel_forward
                ld_model = torch.load(args.load_model)
            elif (ext_dist.my_size > 1 and args.data_set == "terabyte"):
                model_path = args.load_model + "dlrm_distributed_checkpoints_rank" + str(ext_dist.my_local_rank) + ".pt"
                ld_model = torch.load(
                    model_path,
                    # map_location=lambda storage, loc: storage.xpu(0)
                )
            else:
                # NOTE: when targeting inference on single GPU,
                # note that the call to .to(device) has already happened
    ...
    if not args.inference_only:
        ...
        while k < args.nepochs:
            ...
            for j, inputBatch in enumerate(train_ld):
                ...
                if args.mlperf_logging:
                    current_time = time_wrap(use_gpu, use_xpu)
                ...
                if (args.enable_profiling):
                    if use_xpu:
                        ...
                        sort_key = "self_cpu_time_total"
                with (torch.autograd.profiler_legacy.profile(args.enable_profiling, use_xpu=True) if use_xpu else torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA])) as prof1:
                    # forward pass
                    t1 = time.time()
                    if not use_xpu:
                        Z = dlrm_wrap(
                    ...
                    if(k > 0):
                        t2 = time_wrap(use_gpu, use_xpu)
                        total_time += t2 - t1
                        time_collect += t2 - t1
                        time_counts += 1

                if args.enable_profiling and k == 4 and j == 10:
                    if use_xpu:
                        rank_index = ""
                        if ext_dist.my_size > 1:
                            rank_index = "rank_" + str(ext_dist.my_local_rank) + "_"
                        path ='./report/dlrm_training_' + rank_index + 'profiling'
                        torch.save(prof1.key_averages().table(sort_by="self_xpu_time_total"), \
                            path + '.pt')
                        prof1.export_chrome_trace(path + '.json')
                        torch.save(prof1.table(sort_by="id", row_limit=100000), path + '_detail.pt')
                    else:
        ...
        if time_collect == 0:
            ...
            print('dlrm_train latency: ', time_collect, ' s')
            print('dlrm_train avg time: ', time_collect/time_counts, ' s, ant the time counts is :', time_counts)
            print('dlrm_train throughput: ', batch * time_counts /time_collect, ' samples/s')
    else:
        ...
        if args.fp16 and (not args.amp):
            dlrm.half()
        if use_xpu:
            dlrm = torch.xpu.optimize(model=dlrm, dtype=amp_dtype)
        inference(
            args,
            dlrm,
            best_acc_test,
    ...
    if args.save_onnx:
        ...
        # print("output", dlrm_wrap(X_onnx, lS_o_onnx, lS_i_onnx, use_gpu, use_xpu,device))
    ...
    total_time_end = time_wrap(use_gpu, use_xpu)
