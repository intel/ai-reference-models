...
class RNNTGreedyDecoder(TransducerDecoder):
    ...
    def _greedy_decode_batch(self, x, out_lens):
        ...
        max_lens = torch.tensor([max_len-1 for i in range(batch_size)], dtype=torch.int64, device=device)
        # pos 0 of label_tensor is set to _SOS to simplify computation
        # real label start from pos 1
        label_tensor = torch.tensor([self._SOS], device=device).repeat(batch_size, max_len*self.max_symbols)
        ...
        f = x[:, 0, :].unsqueeze(1)
        time_idxs = torch.tensor([0 for i in range(batch_size)], dtype=torch.int64, device=device)

        not_blank = True
        blank_vec = torch.tensor([False for i in range(batch_size)], device=device)
        symbols_added = torch.tensor([0 for i in range(batch_size)], dtype=torch.int64, device=device)
