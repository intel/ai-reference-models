...
echo "RNNT ${PRECISION} inference plain MultiTile=${MULTI_TILE} BS=${BATCH_SIZE}"
...
if [[ ${MULTI_TILE} == "False" ]]; then
    ...
    IPEX_COMPUTE_ENG=1 python -u inference.py --dataset_dir ${DATASET_DIR} --val_manifest ${DATASET_DIR}/librispeech-dev-clean-wav.json --model_toml configs/rnnt_model.toml --ckpt ${WEIGHT_DIR}/rnnt_ckpt.pt -b ${BATCH_SIZE} --seed 0 --steps 30 ${flag} --xpu 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
    ...
    ZE_AFFINITY_MASK=0.0 IPEX_COMPUTE_ENG=1 python -u inference.py --dataset_dir ${DATASET_DIR} --val_manifest ${DATASET_DIR}/librispeech-dev-clean-wav.json --model_toml configs/rnnt_model.toml --ckpt ${WEIGHT_DIR}/rnnt_ckpt.pt -b ${BATCH_SIZE} --seed 0 --steps 30 ${flag} --xpu 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log &
    ZE_AFFINITY_MASK=0.1 IPEX_COMPUTE_ENG=1 python -u inference.py --dataset_dir ${DATASET_DIR} --val_manifest ${DATASET_DIR}/librispeech-dev-clean-wav.json --model_toml configs/rnnt_model.toml --ckpt ${WEIGHT_DIR}/rnnt_ckpt.pt -b ${BATCH_SIZE} --seed 0 --steps 30 ${flag} --xpu 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t1_raw.log &
    wait
    python common/parse_result.py -m $modelname -l ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log -b ${BATCH_SIZE}
    ...
    throughput_unit=$(cat ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf.log | grep "Sun Performance" | awk -F ' ' '{print $4}')
