...
def parse_args():
    ...
    parser.add_argument(
        "--steps",
        default=None,
        help="if not specified do evaluation on full dataset. otherwise only evaluates the specified number of iterations for each worker",
        type=int,
    )
    parser.add_argument(
        "--model_toml",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="if specified will save logits to path",
    )
    parser.add_argument("--seed", default=42, type=int, help="seed")
    parser.add_argument("--wav", type=str, help="absolute path to .wav file (16KHz)")
    parser.add_argument(
        "--warm_up",
        help="warm up steps, will only measure the performance from step=warm_up to step=(steps-warm_up)",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--print-result",
        action="store_true",
        help="print prediction results",
        default=False,
    )
    parser.add_argument(
        "--print_time",
        action="store_true",
        help="print encoder decoder time",
        default=False,
    )
    ...
    parser.add_argument(
        "--mix-precision", action="store_true", help="use bf16", default=False
    )
    parser.add_argument(
        "--profiling", action="store_true", help="do profiling", default=False
    )
    parser.add_argument(
        "--dump_tracing",
        action="store_true",
        help="dump the profiler tracing result",
        default=False,
    )
    parser.add_argument(
        "--sort_by_duration",
        action="store_true",
        help="sort sequence by duration",
        default=False,
    )
    parser.add_argument(
        "--calibration",
    ...
    parser.add_argument(
        "--graph_mode", action="store_true", default=False, help="using Ipex graph mode"
    )
    return parser.parse_args()


def eval(
    data_layer, audio_processor, encoderdecoder, greedy_decoder, labels, multi_gpu, args
):
    ...
    with torch.no_grad():
        ...
        print(
            "Evaluating RNNT: Steps per Epoch {} total Steps {}".format(
        ...
        if args.ipex and args.int8 and args.calibration:
            ...
            for it, data in enumerate(tqdm(data_layer.data_iterator)):
                (
                    ...
                    t_transcript_e,
                    t_transcript_len_e,
                ) = audio_processor(data)

                t_predictions_e, conf = greedy_decoder.decode(
                    t_audio_signal_e, t_a_sig_length_e, args, conf
                )
        ...
        # Inference (vanilla cpu, dnnl fp32 or dnnl int8)
            ...
            if args.steps:
                total_samples = (
                    test_epoches * len(data_layer)
                    + (total_steps - test_epoches * steps_per_epoch) * args.batch_size
                )
            else:
            ...
            print(
                "total time (encoder + decoder, excluded audio processing): ",
                total_time,
                "s",
            )
...
def main(args):
    ...
    if args.ipex:
        ...
        if model.encoder["stack_time"].factor == 2:
            model.encoder["stack_time"] = IPEXStackTime(
                model.encoder["stack_time"].factor
            )
        model = ipex.optimize(
            model,
            dtype=data_type,
    ...
    if args.jit or args.jit_optimize:
        ...
        with torch.autocast("cpu", enabled=args.mix_precision), torch.no_grad():
            model.joint_net = torch.jit.trace(
                model.joint_net,
                torch.randn(
                    args.batch_size,
                    1,
                    1,
                    model_definition["rnnt"]["encoder_n_hidden"]
                    + model_definition["rnnt"]["pred_n_hidden"],
                ),
                check_trace=False,
            )
    ...
    if args.wav is None:
        ...
        if args.steps is not None:
            ...
            total_samples = (
                test_epoches * len(data_layer)
                + (args.steps - test_epoches * step_per_epoch) * args.batch_size
            )
            print(
                "Have {0} examples to eval on.".format(
                    total_samples
                    * (
                        1
                        if not torch.distributed.is_initialized()
