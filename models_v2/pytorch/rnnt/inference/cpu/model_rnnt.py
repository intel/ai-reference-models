...
class RNNT(torch.nn.Module):
    ...
    def predict_batch_origin(self, y, state=None, b=1):
        """
        B - batch size
        U - label length
        H - Hidden dimension size
    ...
    def predict_batch(self, y_in, state=None, b=1):
        """
        B - batch size
        U - label length
        H - Hidden dimension size
        ...
        y = torch.empty(
            [batch_size, y_in.shape[1], embedding_dim],
            dtype=self.prediction["embed"].weight.dtype,
        )

        self._SOS = -1
        torch.ops.torch_ipex.rnnt_embedding(
            self.prediction["embed"].weight,
            y_in,
            y,
            self._SOS,
            batch_size,
