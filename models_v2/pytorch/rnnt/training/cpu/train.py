...
def train(
    ...
    if args.ipex:
        ...
        if args.num_steps is not None:
            total_samples = (
                args.num_steps - args.warmup - start_step
            ) * args.local_batch_size
        else:
            total_samples = (
                len(data_layer) * (args.num_epochs - args.start_epoch)
                - args.warmup * args.local_batch_size * args.world_size
            ) / args.world_size
    ...
    if args.local_batch_size % args.gradient_accumulation_steps != 0:
        raise ValueError(
            "gradient accumulation step {} is not divisible by batch size {}".format(
                args.gradient_accumulation_steps, args.local_batch_size
            )
        )
    ...
    if args.ipex:
        ...
        if args.bf16:
            model, optimizer = ipex.optimize(
                model, dtype=torch.bfloat16, optimizer=optimizer
            )
        else:
            model, optimizer = ipex.optimize(
                model,
...
def parse_args():
    ...
    parser.add_argument(
        "--local_batch_size", default=16, type=int, help="data local batch size"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fp32", action="store_true", default=False, help="enable ipex fp32 path"
    )
    parser.add_argument(
        "--warmup",
        type=int,
        default=0,
        help="if provided, will warm up steps. Only measure the performance from step=warmup",
    )
    parser.add_argument(
