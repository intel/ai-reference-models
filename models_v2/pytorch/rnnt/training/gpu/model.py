...
class AudioPreprocessing(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.optim_level not in [
            ...
            assert (
                False
            ), "unsupported optim level when using intel_extension_for_pytorch"
        else:
            processed_signal = self.featurizer(x)
            processed_length = self.featurizer.get_seq_len(length)
    ...
    def forward(self, x):
        ...
        for idx in range(sh[0]):
            for i in range(self.cutout_rect_regions):
                cutout_rect_x = int(random.uniform(0, sh[1] - self.cutout_rect_freq))
                cutout_rect_y = int(random.uniform(0, sh[2] - self.cutout_rect_time))

                mask[
                    idx,
                    cutout_rect_x : cutout_rect_x + self.cutout_rect_freq,
                    cutout_rect_y : cutout_rect_y + self.cutout_rect_time,
                ] = 1
...
class Jasper(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.jasper_decoder = JasperDecoderForCTC(
            feat_in=kwargs.get("feat_in"), num_classes=kwargs.get("num_classes")
        )
...
class JasperEncoderDecoder(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.acoustic_model = JasperAcousticModel(
            self.jasper_encoder, self.jasper_decoder, kwargs.get("transpose_in", False)
        )
