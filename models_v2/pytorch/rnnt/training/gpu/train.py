...
def evaluator(
    model, data_transforms, loss_fn, greedy_decoder, labels, eval_datasets, logger
):
        ...
        for dataset, frequency, name in eval_datasets:
            ...
            with torch.no_grad():
                ...
                for data in dataloader:
                    ...
                    t_loss_e = loss_fn((t_log_probs_e, x_len), (t_transcript_e, y_len))
                    del t_log_probs_e
        ...
        for data in tqdm(train_dataloader):
            ...
            with (
                ...
                if args.batch_split_factor == 1:
                    ...
                    if args.distributed:
                        f, g, t_log_probs_t = model.module.enc_pred(
                            (
                                (t_audio_signal_t, t_transcript_t),
                                (t_a_sig_length_t, t_transcript_len_t),
                            ),
                        ...
                        f, g, t_log_probs_t = model.enc_pred(
                            (
                                (t_audio_signal_t, t_transcript_t),
                                (t_a_sig_length_t, t_transcript_len_t),
                            ),
                    ...
                    for i in range(args.batch_split_factor):
                        if args.distributed:
                            log_probs = model.module.joint(
                                f_2[i * B_split : (i + 1) * B_split],
                                g_2[i * B_split : (i + 1) * B_split],
                        ...
                        loss = loss_fn(
                            log_probs,
                            t_log_probs_t[i * B_split : (i + 1) * B_split],
                            t_transcript_t[i * B_split : (i + 1) * B_split],
                            t_transcript_len_t[i * B_split : (i + 1) * B_split],
                        )
                        loss /= (
                            args.gradient_accumulation_steps * args.batch_split_factor
                        )
            ...
            if profiling:
                ...
                torch.save(
                    ...
                    "./rnnt_profiling." + profile_name + ".train.pt",
                )
            ...
            if batch_counter % args.gradient_accumulation_steps == 0:
                ...
                batch_counter = 0
                average_loss = 0
                if args.num_steps is not None and step >= (
                    args.num_steps + args.warm_up
                ):
                    break
...
def main(args):
    ...
    if args.batch_split_factor != 1:
        assert (
            args.batch_size % args.batch_split_factor == 0
        ), f"{args.batch_size} % {args.batch_split_factor} != 0"
    ...
    if args.distributed:
        if args.xpu:
            ...
            print(
                "world_size:{}, rank:{}, local_rank:{}, device:{}".format(
                    args.world_size, args.rank, args.local_rank, args.xpu_id
                )
            )
    ...
    if args.lr_decay:
        ...
        fn_lr_policy = lambda s: lr_decay(
...
def parse_args():
    ...
    parser.add_argument(
        "-b", "--batch-size", default=16, type=int, help="data batch size"
    )
    parser.add_argument(
        "--eval_batch_size", default=1, type=int, help="eval data batch size"
    )
    parser.add_argument(
        "--num_epochs",
        default=10,
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="number of epochs until doing evaluation on full dataset",
    )
    parser.add_argument(
        "--test_freq",
        dest="test_frequency",
        default=2,
        type=int,
        help="number of epochs until doing test on full dataset",
    )
    parser.add_argument(
        "--train_freq",
    ...
    parser.add_argument(
        "--tb_path", default=None, type=str, help="where to store tensorboard data"
    )
    parser.add_argument(
        "--histogram",
        default=False,
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="Split batches feed into the joint net",
    )
