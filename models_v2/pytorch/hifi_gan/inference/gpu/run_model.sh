...
if [[ "${PLATFORM}" == "Max" ]]; then
    ...
    PRECISION=${PRECISION:-FP16}
      ...
      echo "HiFI-GAN currently supports on FP16 precision" 
      exit 1
		fi
...
echo "Hifi-Gan ${PRECISION} inference plain MultiTile=${MULTI_TILE} BS=${BATCH_SIZE} Iter=${NUM_ITERATIONS}"
...
if [[ ${MULTI_TILE} == "False" ]]; then
    	...
    	python -u inference.py --checkpoint_file ${CKPT_DIR}/generator_v1 --input_wavs_dir ${DATASET_DIR} --output_dir generated_files --iter ${NUM_ITERATIONS} --dummy --batch_size ${BATCH_SIZE} --dynamic_shape 2>&1 | tee ${OUTPUT_DIR}/${modelname}_${PRECISION}_inf_t0_raw.log
