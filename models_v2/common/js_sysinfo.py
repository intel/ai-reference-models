...
def get_target_pkg_list(target: str, **kwargs):
    try:
        result = subprocess.run(
            ["dpkg", "-S", os.path.realpath(target)],
            check=True,
...
def get_dkms_info(**kwargs):
    ...
    if "dkms" in _components_info:
        ...
        for module in _components_info["dkms"]:
            ...
            if (
                pkg[module]["integrity"] == "broken"
                or _components_info["dkms"][module]["version"]
                not in dpkg[module]["version"]
            ):
                res[module] = _components_info["dkms"][module]
    ...
    try:
        result = subprocess.run(
            ["dkms", "status", "-k", platform.release()],
            text=True,
            check=True,
            stdout=subprocess.PIPE,
            stderr=None if verbose(**kwargs) else subprocess.DEVNULL,
...
def get_dpkg_info(**kwargs):
    ...
    if not quiet(**kwargs):
        print(
            "info: calling dpkg-query and dpkg --verify (might be time consuming)...",
            file=sys.stdout,
        )

    dpkg = {"file-type": "dpkg", "system": get_system(), "content": {}}
...
def get_pytorch_info(**kwargs):
    if not quiet(**kwargs):
        print(
            "info: importing PyTorch components (might be time consuming)...",
            file=sys.stdout,
        )
    try:
        import torch
    ...
    res = {"pytorch": {"name": "PyTorch", "version": str(torch.__version__)}}
    ...
    try:
        ...
        if verbose(**kwargs):
            print("warning: " + str(e), file=sys.stderr)
            print(
                "warning: cannot import 'intel_extension_for_pytorch'", file=sys.stderr
            )
...
def get_svrinfo(**kwargs):
    ...
    if "svrinfo" not in _components_info:
        if not quiet(**kwargs):
            print(
                "info: calling svr-info (might be time consuming)...", file=sys.stdout
            )

        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                result = subprocess.run(
                    ["svr-info", "-format", "json", "-output", tmpdir],
                    check=True,
...
def get_software(**kwargs):
    res = {
        ...
        "kernel": {"name": "kernel", "version": platform.release()},
        "python": {"name": "Python", "version": platform.python_version()},
    }
...
def get_i915_prelim(card, **kwargs):
    ...
    if prelim_uapi_version == "":
        return {}
    res = {"prelim.uapi.version": prelim_uapi_version}
    return res
...
def get_gpu_info(card, **kwargs):
    ...
    if vendor_id.lower() == "0x10de".lower():
        res |= {"name": pci_id, "vendor": "NVIDIA Corporation"}
        ...
        res |= {"name": pci_id, "vendor": "ASPEED Technology"}
        ...
        res |= {"name": pci_id, "vendor": vendor_id}
    ...
    if "lshw" in _components_info:
        ...
        for cpu in cpus:
            ...
            if "product" in cpu_info and "vendor" in cpu_info:
                ...
                cpu_res |= {"name": cpu_info["product"], "vendor": cpu_info["vendor"]}
                ...
                for c in cpu_list:
                    ...
                    governor = read_sysfs_file(
                        path + "/cpufreq/scaling_governor", **kwargs
                    )
                    min_frequency = read_sysfs_file(
                        path + "/cpufreq/scaling_min_freq", **kwargs
                    )
                    max_frequency = read_sysfs_file(
                        path + "/cpufreq/scaling_max_freq", **kwargs
                    )
                ...
                if "configuration" in cpu_info:
                    for c in cpu_info["configuration"]:
                        if c == "cores":
                            cpu_res["configuration"] |= {
                                "cores.total": int(cpu_info["configuration"][c])
                            }
                        elif c == "enabledcores":
                            cpu_res["configuration"] |= {
                                "cores.enabled": int(cpu_info["configuration"][c])
                            }
                        elif c == "threads":
                            cpu_res["configuration"] |= {
                                c: int(cpu_info["configuration"][c])
                            }
                        else:
                            cpu_res["configuration"] |= {
                                c: cpu_info["configuration"][c]
                            }
            ...
            if "children" in memory:
                ...
                for child in memory["children"]:
                    ...
                    if not slot:
                        ...
                        continue
                    if (
                        child["product"] != slot["name"]
                        or child["vendor"] != child["vendor"]
                    ):
                        same_mem = False
                ...
                if same_mem:
                    ...
                    freq = (
                        freq_match.group(1) if freq_match.group(1) is not None else ""
                    )
                    mem_res |= {
                        "name": slot["name"],
                        "vendor": slot["vendor"],
                        "description": str(slots_taken)
                        + "x"
                        + str(slot["size"]["value"])
                        + slot["size"]["units"]
                        + freq,
...
def get_hardware(**kwargs):
    ...
    res = {
        "product": {
            "name": read_sysfs_file(
                "/sys/devices/virtual/dmi/id/product_name", **kwargs
            ),
            "version": read_sysfs_file(
                "/sys/devices/virtual/dmi/id/product_version", **kwargs
            ),
        },
        "board": {
            "name": read_sysfs_file("/sys/devices/virtual/dmi/id/board_name", **kwargs),
            "vendor": read_sysfs_file(
                "/sys/devices/virtual/dmi/id/board_vendor", **kwargs
            ),
            "version": read_sysfs_file(
                "/sys/devices/virtual/dmi/id/board_version", **kwargs
            ),
            "bios": {
                "date": read_sysfs_file(
                    "/sys/devices/virtual/dmi/id/bios_date", **kwargs
                ),
                "vendor": read_sysfs_file(
                    "/sys/devices/virtual/dmi/id/bios_vendor", **kwargs
                ),
                "version": read_sysfs_file(
                    "/sys/devices/virtual/dmi/id/bios_version", **kwargs
                ),
            },
        },
    }
    ...
    for card in drm:
        if re.match("card[0-9]*$", card):
            res |= {
                re.sub("card", "gpu", card): get_gpu_info(
                    "/sys/class/drm/" + card, **kwargs
                )
            }
...
def get_parser():
    ...
    parser.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Be quiet and suppress messages to stdout",
    )
    ...
    group.add_argument(
        ...
        "--all",
        action="store_true",
        help="collect information for all known components",
    )
    group.add_argument(
        "--verify",
    ...
    group.add_argument(
        "--cuda", action="store_const", const=True, help="collect information for CUDA"
    )
    group.add_argument(
        "--dkms", action="store_const", const=True, help="collect information for DKMS"
    )
    group.add_argument(
        "--docker",
        action="store_const",
        const=True,
        help="collect information for Docker",
    )
    group.add_argument(
        "--dpkg",
        action="store_const",
        const=True,
        help="collect information for some key DPKG packages",
    )
    group.add_argument(
        "--lshw", action="store_const", const=True, help="collect information with lshw"
    )
    group.add_argument(
        "--pytorch",
        action="store_const",
        const=True,
        help="collect information for Pytorch",
    )
    group.add_argument(
        "--svrinfo",
        action="store_const",
        const=True,
        help="collect information with svr-info (should be in PATH)",
    )
    group.add_argument(
        "--xpu", action="store_const", const=True, help="collect information for xpu"
    )
