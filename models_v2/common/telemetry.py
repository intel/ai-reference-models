    ...
    if platform in ["Flex", "Max"]:
        try:
            smi_process = subprocess.Popen(
                "xpu-smi dump -d 0 -m 0,1,2,3,9,10,11,18,22,24,26,27,35,36,19,20".split(),
            ...
            print(
                'error: "xpu-smi" for telemetry capture does not exist', file=sys.stderr
            )
            return None
    elif platform == "CUDA":
        try:
            smi_process = subprocess.Popen(
                [
                    ...
                    "0",
                    "-f",
                    "{0}/nvidia_smi_dump.csv".format(output_dir),
                ]
            )
        except:
            print(
                'error: "nvidia-smi" for telemetry capture does not exist',
                file=sys.stderr,
            )
            return None
    return smi_process
