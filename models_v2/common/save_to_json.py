...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory",
        action="store_true",
        help="if specified, target argument points to a directory within which key-values pairs will be saved to all matched json files",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="if specified and operating in directory mode, key-values pairs will be saved to all matched json files recursively",
    )
    parser.add_argument(
        "--pattern",
        default=None,
        help="if operating in directory mode, only json files that match this regex expression will be modified",
    )
    parser.add_argument(
        "--amend", action="store_true", help="amend data to output file"
    )
    parser.add_argument("--indent", default=None, help="indent for json.dump()")
    parser.add_argument(
        "target", help="JSON file (or directory if in directory mode) to store output"
    )
    parser.add_argument(
        "keys",
        metavar="key=value",
        help="key/value pair to amend (key.key=value for nested keys)",
        nargs="+",
    )
