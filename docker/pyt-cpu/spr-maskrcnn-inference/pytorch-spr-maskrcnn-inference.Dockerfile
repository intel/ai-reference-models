...
RUN pip install matplotlib Pillow pycocotools && \
    pip install yacs opencv-python cityscapesscripts defusedxml
...
COPY quickstart/object_detection/pytorch/maskrcnn/inference/cpu/inference_realtime.sh quickstart/inference_realtime.sh
COPY quickstart/object_detection/pytorch/maskrcnn/inference/cpu/inference_throughput.sh quickstart/inference_throughput.sh
COPY quickstart/object_detection/pytorch/maskrcnn/inference/cpu/accuracy.sh quickstart/accuracy.sh 

# RUN cd /workspace/pytorch-spr-maskrcnn-inference/models/object_detection/pytorch/maskrcnn/maskrcnn-benchmark && \
#     pip install setuptools==58.2.0

RUN apt-get update && apt-get install -y python3.10-dev && \
    cd /workspace/pytorch-spr-maskrcnn-inference/models/object_detection/pytorch/maskrcnn/maskrcnn-benchmark && \
