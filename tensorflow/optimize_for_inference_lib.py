...
def optimize_for_inference(input_graph_def, input_node_names=None, output_node_names=None,cuts=None,
  ...
  Returns:
    An optimized version of the input graph or optimized cut graphs.
  """
  ensure_graph_is_valid(input_graph_def)
  if cuts:
    optimized_graphs=[]
    # loop over input, output nodes for graph cut
    for ind,graph_cut in enumerate(cuts):
      optimized_graph_def = input_graph_def
      input_node_names,output_node_names=graph_cut
      # graph cut
      ...
      # with gfile.GFile('./graph_cut+'+str(ind)+'.pbtxt', "wb") as f:
      ...
      # append cut graphs to list
      optimized_graphs.append(optimized_graph_def)
      return optimized_graphs
  else:
    # usual tf execution flow
    optimized_graph_def = strip_unused_lib.strip_unused(
