...
def train(args, train_dataset, model, tokenizer, optimizer):
    ...
    set_seed(args)
    #torch_ipex_cpp.manual_seed(args.seed)
                    ...
                    with torch.autograd.profiler.record_function("clip_grad_norm"):
                        if args.fp16:
                            global_norm = torch.nn.utils.clip_grad_norm_(
                             ...
                             global_norm = tpp.optim.clip_grad_norm_(
...
def main():
    ...
    if args.use_tpp:    
        model, optimizer = ipex.fast_bert(model, dtype=torch.bfloat16 if args.tpp_bf16 else torch.float, optimizer=optimizer, unpad = args.unpad)
    # import pdb 
    # pdb.set_trace()
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            if args.use_tpp:
                model = ipex.fast_bert(model, dtype=torch.bfloat16 if args.tpp_bf16 else torch.float, unpad = args.unpad)
            # for n, p in model.named_parameters():
            #     print(f"{n}: {p.dtype}")
