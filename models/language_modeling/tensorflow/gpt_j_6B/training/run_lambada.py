...
class DataTrainingArguments:
    ...
    dataset_name: Optional[str] = field(
        default="lambada", metadata={"help": "The name of the dataset to use (via the datasets library)."}
    )
...
@dataclass
class MTFTrainingArguments(TFTrainingArguments):
    """
    Arguments pertaining to which model/config/tokenizer we are going to fine-tune from.
    """
    precision: str = field( default="fp32",
        metadata={"help": " The precision for model to run: fp32, bfloat16, fp16" },
    )
    intra_op_parallelism_threads: int = field(
        default=56,
        metadata={
            "help": (
                "The maximum intra op threads to use"
            )
        },
    )
    inter_op_parallelism_threads: int = field(
        ...
        metadata={
            "help": (
                "The maximum inter op threads to use"
            )
        },
    )
    profile: bool = field(
        default=False, metadata={"help": "Generate profile output for the model."}
    )
    profile_dir : str = field(default="./tensorboard_logs",
        metadata={"help": "Set Tensorboard profile dir "},
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, MTFTrainingArguments))
    ...
    elif training_args.precision == "fp16" :
      tf.keras.mixed_precision.set_global_policy('mixed_float16')
    tf.config.threading.set_inter_op_parallelism_threads(training_args.inter_op_parallelism_threads)
    tf.config.threading.set_intra_op_parallelism_threads(training_args.intra_op_parallelism_threads)
