...
class DataTrainingArguments:
    ...
    task_name: str = field( default="MRPC",
    ...
    profile_dir : str = field(default="./tensorboard_logs",
        metadata={"help": "Enable Tensorboard profile"},
    )
    ...
    checkpoint_save_freq: int = field(
        ...
        metadata={
            "help": (
                "The frequency with which checkpoint should be saved.!"
            )
        },
    )
...
class ModelArguments:
    ...
    model_name_or_path: str = field( default = "EleutherAI/gpt-j-6B",
    ...
    steps : int = field(
        ...
        metadata={
            "help": (
                "The maximum steps to run the model"
            )
        },
    )
...
def main():
    ...
    with training_args.strategy.scope():
        ...
        if data_args.profile:
            callbacks.append(tf.keras.callbacks.TensorBoard(log_dir=data_args.profile_dir,
                                                            update_freq=5,
                                                            profile_batch=13,
                                                            write_graph=True,
        ...
        if training_args.do_train:
            ...
            if model_args.steps :
              num_train_steps = int(model_args.steps)
              epochs = 1
            else :
              num_train_steps = len(tf_data["train"])
