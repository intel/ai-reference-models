...
auto_mixed_precision_options.allowlist_add = (
    "MatMul,BiasAdd,BatchMatMulV2,Mul,AddV2,Softmax,LayerNorm,Gelu"
)
auto_mixed_precision_options.inferlist_remove = (
    "MatMul,BiasAdd,BatchMatMulV2,Mul,AddV2,Softmax,LayerNorm,Gelu"
)
...
flags.DEFINE_string(
    "mode", "benchmark", "One of three options: 'benchmark'/'profile'/'accuracy'."
)
...
flags.DEFINE_integer(
    "accum_steps", 1, "Accumulation steps for batch size greater than 32."
)
...
flags.DEFINE_integer(
    "inter_op_parallelism_threads", 1, "Setting inter op for the model"
)

flags.DEFINE_integer(
    "intra_op_parallelism_threads", 28, "Setting inter op for the model"
)

flags.DEFINE_bool(
    "profile",
    False,
    "[Optional] To enable Tensorflow profile hook."
...
flags.DEFINE_bool(
    "optimized_softmax", False, "[Optional] If true, use optimized softmax op in model."
)

flags.DEFINE_integer("warmup_steps", 10, "[Optional] Number of warmup steps.")

flags.DEFINE_integer("steps", 30, "[Optional] Number of benchmark steps.")

flags.DEFINE_bool(
    "weight_sharing", False, "Simulate weight sharing across multiple instances."
)
flags.DEFINE_integer("num_cores_per_socket", None, "Number of cores per socket.")
...
class UpdateGlobalStepHook(session_run_hook.SessionRunHook):
    ...
    def begin(self):
        ...
        if self._global_step_tensor is None:
            raise RuntimeError(
                "Global step should be created to use UpdateGlobalStepHook."
            )
        tf.compat.v1.get_default_graph()._unsafe_unfinalize()
        self._updated_global_step = state_ops.assign_add(
            self._global_step_tensor, 1, use_locking=True
        )
            ...
            def compute_loss(logits, positions):
                ...
                loss = -tf.reduce_mean(
                    input_tensor=tf.reduce_sum(
                        input_tensor=one_hot_positions * log_probs, axis=-1
                    )
                )
                return loss
...
def do_benchmark():
    ...
    if FLAGS.weight_sharing:
        if not FLAGS.num_cores_per_socket:
            raise ValueError("Weight sharing requires number of cores per socket info.")
        if not os.getenv("OMP_NUM_THREADS"):
            raise ValueError(
                "Weight sharing requires OMP_NUM_THREADS environment variable setting."
            )
        num_instances = int(
            FLAGS.num_cores_per_socket // int(os.getenv("OMP_NUM_THREADS"))
        )
    else:
        num_instances = 1
    if num_instances < 1:
    ...
    def create_feed_dict():
        ...
        paragraph_length = np.random.randint(MIN_PARAGRAPH_LENGTH, MAX_PARAGRAPH_LENGTH)
        input_ids = np.random.randint(
            1, VOCAB_SIZE, [question_length + paragraph_length]
        )
        ...
        segment_ids = np.pad(
            segment_ids,
            (question_length, MAX_SEQ_LENGTH - (question_length + paragraph_length)),
        )
    ...
    def run_model(sess, tid):
        all_tensor_name = [
            tensor.name for op in sess.graph.get_operations() for tensor in op.values()
        ]
        ...
        if (
            candidate_outputs_1[0] in all_tensor_name
            and candidate_outputs_1[1] in all_tensor_name
        ):
            outputs = candidate_outputs_1

        if (
            candidate_outputs_2[0] in all_tensor_name
            and candidate_outputs_2[1] in all_tensor_name
        ):
            outputs = candidate_outputs_2
...
def main(_):
    ...
    print("Running with " + FLAGS.precision + " precision")
        ...
        if FLAGS.profile == True:
            ...
            hooks = [
                tf.compat.v1.train.ProfilerHook(
                    save_steps=3, output_dir=FLAGS.output_dir, show_memory=True
                )
            ]
            estimator.train(
                input_fn=train_input_fn, max_steps=num_train_steps, hooks=hooks
            )
        else:
    ...
    if FLAGS.do_predict:
        ...
        if FLAGS.mode == "benchmark":
            hooks = [UpdateGlobalStepHook(), StopAtStepHook(total_steps)]
        elif FLAGS.mode == "profile":
            hooks = [
                UpdateGlobalStepHook(),
        ...
        for result in estimator.predict(
            ...
            if FLAGS.mode in ("benchmark", "profile"):
                if num_processed_examples % 10 == 0:
                    tf.compat.v1.logging.info(
                        "Processed #examples: %d" % (num_processed_examples)
                    )
