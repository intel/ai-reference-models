...
def create_optimizer(
    ...
    if accum_steps > 1:
        ...
        current_step = tf.compat.v1.get_variable(
            name="current_step",
            shape=[],
            dtype=tf.int32,
            trainable=False,
        ...
        apply_grads = tf.cast(
            tf.math.equal(current_step % accum_steps, 0), dtype=tf.bool
        )
        ...
        apply_step = tf.identity(
            tf.cast(tf.math.equal(current_step % accum_steps, 0), dtype=tf.bool),
            name="apply_step",
        )
        update_op = tf.cond(
            apply_step, lambda: applyGrads(accum_vars, current_step), lambda: tf.no_op()
        )

        new_global_step = tf.cond(
            apply_step, lambda: global_step + 1, lambda: global_step
        )
...
class AdamWeightDecayOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def apply_gradients(
        self, grads_and_vars, fine_tuning=True, global_step=None, name=None
    ):
        """See base class."""
        assignments = []
        for grad, param in grads_and_vars:
            ...
            if fine_tuning:
                ...
                next_m = bf.multiply(self.beta_1, m) + bf.multiply(
                    1.0 - self.beta_1, grad
                )
                next_v = bf.multiply(self.beta_2, v) + bf.multiply(
                    1.0 - self.beta_2, tf.square(grad)
                )
            ...
            if fine_tuning:
                ...
                param, m, v = (
                    tf.cast(next_param, param.dtype),
                    tf.cast(m, m.dtype),
                    tf.cast(v, v.dtype),
                )
