...
flags.DEFINE_integer(
    "inter_op_parallelism_threads", 2, "Setting inter op for the model"
)

flags.DEFINE_integer(
    "intra_op_parallelism_threads", 27, "Setting inter op for the model"
)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        if FLAGS.mode == "benchmark":
            hooks = [UpdateGlobalStepHook(), StopAtStepHook(30)]
        ...
        if FLAGS.mode in ("benchmark", "profile"):
            ...
            print(
                "Elapsed time: %f num processed examples: %d threshod_examples: %d"
                % (end - start, num_processed_examples, threshod_examples)
            )
            print(
                "throughput((num_processed_examples-threshod_examples)/Elapsedtime): %3.2f"
                % (
                    float(num_processed_examples - threshod_examples)
                    / float(end - start)
                )
            )
