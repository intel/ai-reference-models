...
class LoggerHook(tf.estimator.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._step > self._warmup:
            ...
            print(
                "Throughput is %.2f samples/s" % (self.batch_size * 1000 / time_takes)
            )
        else:
...
def create_model(
    ...
    output_weights = tf.compat.v1.get_variable(
        "cls/classifier/output_weights",
        [num_labels, hidden_size],
        initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02),
    )

    output_bias = tf.compat.v1.get_variable(
        "cls/classifier/output_bias",
        [num_labels],
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if init_checkpoint:
            (
                ...
                tvars, init_checkpoint, "Classifier"
            )
            if use_tpu:
        ...
        if FLAGS.profile:
            ...
            hooks = [
                tf.estimator.ProfilerHook(
                    save_steps=10, output_dir=FLAGS.output_dir, show_memory=False
                )
            ]

        result = estimator.evaluate(
            input_fn=eval_input_fn, steps=eval_steps, hooks=hooks
        )
