...
class LoggerHook(tf.estimator.SessionRunHook):
    ...
    def before_run(self, run_context):
        ...
        if self.run_profile:
            return session_run_hook.SessionRunArgs(requests, options=opts)
        else:
            return session_run_hook.SessionRunArgs(requests)
    ...
    def end(self, run_context):
        ...
        print(
            "Time spent per iteration (after warmup): %.2f ms"
            % (self._total_duration / (self._step - self._warmup))
        )
...
def create_model_top(
    ...
    if frozen_graph and not is_training:
        ...
        outputs = ["unstacked_logits:0", "unstacked_logits:1"]
    ...
    if FLAGS.do_predict:
        ...
        if FLAGS.mode == "benchmark":
            hooks = [
                ...
                LoggerHook(FLAGS.predict_batch_size, False),
            ]
        ...
        if FLAGS.mode in ("benchmark", "profile"):
            for result in estimator.predict(
                predict_input_fn, yield_single_examples=False, hooks=hooks
            ):
                continue
