...
tf.compat.v1.flags.DEFINE_bool(
    "saved_model", False, "whether export saved model or not"
)
...
class ClassifierExporter:
    def __init__(
        self, output_dir: str, task_name: str, bert_config: str, max_seq_length: int
    ):
    ...
    def export_frozen_graph(self, frozen_graph_name="frozen_graph.pb"):
        ...
        graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
            self.session, graph_def, outputs_name
        )
...
def main(_):
    ...
    with ClassifierExporter(
        FLAGS.output_dir, FLAGS.task_name, FLAGS.bert_config_file, FLAGS.max_seq_length
    ) as exporter:
