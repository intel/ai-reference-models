...
class BertConfig(object):
    ...
    def __init__(
        ...
        Args:
          ...
          precision: To enable fp32, bfloat16, or float16 based training
        """
        ...
        self.new_fp16_scope = new_fp16_scope
    ...
    def set_additional_options(
        self, precision, experimental_gelu, optimized_softmax, amp
    ):
        if precision:
            self.precision = precision
        if experimental_gelu:
            ...
            tf.compat.v1.logging.info(
                "Set experimental gelu to false(default) if you see issues. "
            )
        ...
        if amp:
            self.amp = amp
            tf.compat.v1.logging.info("Auto Mixed Precision enabled for model.")

    def new_scope_settings(self, new_fp16_scope):
        if new_fp16_scope:
            self.new_fp16_scope = new_fp16_scope
            tf.compat.v1.logging.info("Float16 scope set for model..!")
...
class BertModel(object):
    ...
    def __init__(
        ...
        """
        # Flags for FP16 CPU
        self.fp16_scope = False
        if config.precision == "fp16" and not config.amp:
            bf.set_global_precision(tf.float16)
            if config.new_fp16_scope:
                self.fp16_scope = True

        if config.amp:
            self.amp = True

        bf.set_global_flags(
            optimized_softmax=config.optimized_softmax,
    ...
    def get_pooled_output(self):
        """
        In float16 enabled execution, with only model covered in float16 scope,
        return the output in float32. Other cases return as is
        """
        if self.fp16_scope == True:
    ...
    def get_sequence_output(self):
        ...
        Returns:
          ...
          In float16 enabled execution through the Keras Mixed policy,
          with only model covered in float16 scope,
          return the output in float32. Other cases return as is
        """
        if self.fp16_scope == True:
    ...
    def get_embedding_output(self):
        ...
        """
        if self.fp16_scope == True or self.amp:
