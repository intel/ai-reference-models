    ...
    if accum_steps > 1:
        ...
        if use_multi_cpu:
            ...
            if amp:
                ...
                grads = tf.gradients(
                    loss * math_ops.cast(loss_scale, loss.dtype), tvars
                )
        ...
        if amp:
                ...
                (
                    ...
                    if grad is None
                        ...
                        indexed_slices.IndexedSlices(
                            grad.values * loss_scale_inverse,
                            grad.indices,
                            grad.dense_shape,
                        )
                        if isinstance(grad, indexed_slices.IndexedSlices)
                        else grad * loss_scale_inverse
                    )
                )
                for grad in grads
            ]
            ...
            def update_step():
                ...
                new_global_step = global_step + 1
                new_global_step = tf.identity(
                    new_global_step, name="global_step_update"
                )
                train_op = tf.group(train_op, [global_step.assign(new_global_step)])
                return train_op
