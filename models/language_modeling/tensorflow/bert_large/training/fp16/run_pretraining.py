...
flags.DEFINE_integer(
    "intra_op_parallelism_threads", 20, "intra_op_parallelism_threads."
)
...
flags.DEFINE_bool(
    "new_fp16_scope",
    True,
    "[Optional] To change scopes for float16."
    " Set this to False to enable float16 scope for optimizer",
)
...
flags.DEFINE_bool(
    "optimized_softmax",
    False,
    "[Optional] If true, use experimental fp16 softmax for internal softmaxes inside each layer."
...
def model_fn_builder(
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if bert_config.precision == "float16" and bert_config.new_fp16_scope == True:
            with tf.compat.v1.tpu.float16_scope():
                bf.set_global_precision(tf.float16)
                tf.compat.v1.logging.info("*** New float16 scope set***")
...
def main(_):
    ...
    bert_config.new_scope_settings(FLAGS.new_fp16_scope)
    bert_config.set_additional_options(
        FLAGS.precision, FLAGS.experimental_gelu, FLAGS.optimized_softmax
    )
    ...
    if bert_config.precision == "float16":
        tf.compat.v1.logging.info("INFO: BERT float16 training....!")
    else:
        tf.compat.v1.logging.info("INFO: BERT float32 training....!")

    if bert_config.precision == "float16" and bert_config.new_fp16_scope == False:
        with tf.compat.v1.tpu.float16_scope():
            bf.set_global_precision(tf.float16)
            tf.compat.v1.logging.info("*** Old float16 scope set***")
        ...
        if FLAGS.profile:
            ...
            hooks.append(
                tf.compat.v1.train.ProfilerHook(
                    save_steps=3, output_dir=FLAGS.output_dir, show_memory=False
                )
            )

        estimator.train(
            input_fn=train_input_fn, max_steps=FLAGS.num_train_steps, hooks=hooks
        )
