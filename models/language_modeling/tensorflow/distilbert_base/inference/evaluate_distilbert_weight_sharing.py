...
class distilbert_base_inference:
    ...
    def __init__(self):
        ...
        arg_parser.add_argument("--batch-size",
                                ...
                                default=32,
                                dest= "batch_size")
        arg_parser.add_argument('-p', "--precision", type=str, default='bfloat16',
    ...
    def run(self):
        ...
        def run_model(sess, tid, feed_dict):
            ...
            for batch_id in range(steps):
                tf_logging.warn('\n---> Thread {0}: Start iteration {1}'.format(tid, str(batch_id)))
                start = time.time()
                _ = sess.run(output, feed_dict=feed_dict)
                end = time.time()
                tf_logging.warn('\n---> Thread {0}: Stop iteration {1}'.format(tid, str(batch_id)))
                time_consume = end - start
                total_time += time_consume
            time_per_batch = total_time / float(steps)
