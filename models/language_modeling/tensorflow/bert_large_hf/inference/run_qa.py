...
class DataTrainingArguments:
    ...
    train_eval_warmup_steps: Optional[int] = field(
        default=10,
        metadata={"help": "Number of warmup steps for training and eval."},
    )
    steps: Optional[int] = field(
        default=30,
        metadata={"help": "Number of steps for training and eval."},
    )
...
class TimingCallback(keras.callbacks.Callback):
    def __init__(self, batch_size, warmup_steps, steps):
        ...
        self.num_processed_examples = 0
        self.warmup_steps = warmup_steps
        self.steps = steps

    def on_predict_batch_begin(self, iteration, logs={}):
        if iteration == self.warmup_steps:
            # Start timer once warmup steps are done
        ...
        if training_args.do_eval:
            ...
            if model_args.mode == "benchmark":
                ...
                if data_args.batch_size == 1:
                    eval_latency = (data_args.batch_size / eval_throughput) * 1000
