...
parser.add_argument(
    "--batch_size", type=int, default=32, help="Specify inference batch size."
)
parser.add_argument(
    "--num_inter_threads",
    type=int,
...
parser.add_argument(
    "--src_vocab_file", type=str, required=True, help="Specify source vocab file."
)
parser.add_argument(
    "--tgt_vocab_file", type=str, required=True, help="Specify target vocabulary file."
)
parser.add_argument(
...
def read_source_sentences(inference_input_file):
    ...
    with codecs.getreader("utf-8")(
        tf.io.gfile.GFile(inference_input_file, mode="rb")
    ) as f:
    ...
    if tf.io.gfile.exists(vocab_file):
        ...
        if vocab[0] != unk or vocab[1] != sos or vocab[2] != eos:
            ...
            with codecs.getwriter("utf-8")(
                tf.io.gfile.GFile(new_vocab_file, "wb")
            ) as f:
                for word in vocab:
                    f.write("%s\n" % word)
...
if __name__ == "__main__":
    ...
    with graph.as_default():
        ...
        sample_words_tensor = graph.get_tensor_by_name(
            "hash_table_Lookup_1/LookupTableFindV2:0"
        )
