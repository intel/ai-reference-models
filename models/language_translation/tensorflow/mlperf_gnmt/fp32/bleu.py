...
class RunningBLEUScorer:
    ...
    def add_sentence_with_multiple_refs(self, references, translation):
        ...
        (
            new_matches_by_order,
            new_possible_matches_by_order,
        ) = self._get_ngram_match_values(
            merged_ref_ngram_counts, translation_ngram_counts, len(translation)
        )
    ...
    def calc_BLEU_score(self):
        ...
        if min(precisions) > 0:
            p_log_sum = sum((1.0 / self.max_order) * math.log(p) for p in precisions)
