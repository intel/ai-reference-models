...
def decode_and_evaluate(
    ...
    with codecs.getwriter("utf-8")(tf.io.gfile.GFile(trans_file, mode="wb")) as trans_f:
        trans_f.write("")  # Write empty string to ensure file is created.
        num_translations_per_input = min(num_translations_per_input, beam_width)

        print(
            "  Running inference with beam_width %g, num translations per input %d. "
        ...
        for sent_id in range(batch_size):
            translation = get_translation(nmt_outputs[0], sent_id, tgt_eos="</s>")
            if mode == "accuracy":
                    ...
                    for sent_id in range(batch_size):
                        for beam_id in range(num_translations_per_input):
                            translation = get_translation(
                                nmt_outputs[beam_id], sent_id, tgt_eos="</s>"
                            )
                            if mode == "accuracy":
                                trans_f.write((translation + b"\n").decode("utf-8"))
