...
def _bleu(ref_file, trans_file, subword_option=None):
    ...
    for reference_filename in ref_files:
        with codecs.getreader("utf-8")(
            tf.io.gfile.GFile(reference_filename, "rb")
        ) as fh:
...
def _rouge(ref_file, summarization_file, subword_option=None):
    ...
    hypotheses = []
    with codecs.getreader("utf-8")(tf.io.gfile.GFile(summarization_file, "rb")) as fh:
        for line in fh:
...
def _moses_bleu(multi_bleu_script, tgt_test, trans_file, subword_option=None):
    ...
    if subword_option == "bpe":
        ...
        if not os.path.exists(debpe_tgt_test):
            subprocess.call(
                shlex.split("cp %s %s" % (tgt_test, debpe_tgt_test)), shell=False
            )
            subprocess.call(
                shlex.split("sed s/@@ //g %s" % (debpe_tgt_test)), shell=False
            )
