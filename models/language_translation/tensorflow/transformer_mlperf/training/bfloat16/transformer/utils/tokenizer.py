...
class Subtokenizer(object):
    ...
    def __init__(self, vocab_file, reserved_tokens=None):
        ...
        tf.compat.v1.logging.info(
            "Initializing Subtokenizer from file %s." % vocab_file
        )
    ...
    def init_from_files(
        ...
        if tf.io.gfile.exists(vocab_file):
            ...
            mlperf_log.transformer_print(
                key=mlperf_log.PREPROC_VOCAB_SIZE, value=len(subtoken_list)
            )
...
def _generate_subtokens_with_target_vocab_size(
    token_counts, alphabet, target_size, threshold, min_count=None, reserved_tokens=None
):
    ...
    def bisect(min_val, max_val):
        ...
        tf.compat.v1.logging.info(
            "Binary search: trying min_count=%d (%d %d)" % (cur_count, min_val, max_val)
        )
...
def _generate_subtokens(
    token_counts, alphabet, min_count, num_iterations=4, reserved_tokens=None
):
