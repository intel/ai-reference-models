...
def padded_sequence_accuracy(logits, labels):
    ...
    with tf.compat.v1.variable_scope(
        ...
        padded_labels = tf.cast(labels, dtype=tf.int32)
        not_correct = (
            tf.cast(tf.not_equal(outputs, padded_labels), dtype=tf.float32) * weights
        )
        axis = list(range(1, len(outputs.get_shape())))
        correct_seq = 1.0 - tf.minimum(
            1.0, tf.reduce_sum(input_tensor=not_correct, axis=axis)
        )
...
def rouge_l_fscore(predictions, labels):
    ...
    rouge_l_f_score = tf.compat.v1.py_func(
