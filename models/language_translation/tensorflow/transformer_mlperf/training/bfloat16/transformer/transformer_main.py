...
def model_fn(features, labels, mode, params):
    ...
    with tf.compat.v1.variable_scope("model"):
        ...
        loss = tf.reduce_sum(input_tensor=xentropy * weights) / tf.reduce_sum(
...
def get_train_op(loss, params):
    ...
    with tf.compat.v1.variable_scope("get_train_op"):
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_LR_WARMUP_STEPS, value=params.learning_rate_warmup_steps
        )
        learning_rate = get_learning_rate(
            params.learning_rate, params.hidden_size, params.learning_rate_warmup_steps
        )
        ...
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_NAME, value=mlperf_log.LAZY_ADAM
        )
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_HP_ADAM_BETA1, value=params.optimizer_adam_beta1
        )
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_HP_ADAM_BETA2, value=params.optimizer_adam_beta2
        )
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_HP_ADAM_EPSILON, value=params.optimizer_adam_epsilon
        )
...
def train_schedule(
    ...
    **Step vs. Epoch vs. Iteration**
    ...
    if FLAGS.save_profile == "Yes":
        profile_hooks = [
            tf.compat.v1.train.ProfilerHook(save_steps=1, output_dir=FLAGS.profile_dir)
        ]  # the json file
    ...
    for i in xrange(train_eval_iterations):
        ...
        if single_iteration_train_epochs is not None:
            mlperf_log.transformer_print(
                key=mlperf_log.TRAIN_EPOCH, value=i * single_iteration_train_epochs + 1
            )
        ...
        estimator.train(
            dataset.train_input_fn, steps=single_iteration_train_steps, hooks=hooks
        )
        ...
        if FLAGS.do_eval == "Yes":
            ...
            print(
                "Evaluation results (iter %d/%d):" % (i + 1, train_eval_iterations),
                eval_results,
            )
        ...
        if evaluate_bleu:
            ...
            mlperf_log.transformer_print(
                key=mlperf_log.EVAL_ACCURACY, value=uncased_score
            )
...
def main(_):
    ...
    if FLAGS.params == "base":
        ...
        raise ValueError(
            "Invalid parameter set defined: %s."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="the intra op parallelism thread to use",
        metavar="<INTRA>",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="print_iteration to print loss and timing",
        metavar="<PI>",
    )
