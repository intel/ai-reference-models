...
class SequenceBeamSearch(object):
    ...
    def search(self, initial_ids, initial_cache):
        ...
        finished_state = tf.while_loop(
            cond=self._continue_search,
            body=self._search_step,
            loop_vars=[state],
    ...
    def _continue_search(self, state):
        ...
        finished_scores *= tf.cast(
            finished_flags, dtype=tf.float32
        ...
        lowest_finished_scores += (
            1.0 - tf.cast(finished_batches, dtype=tf.float32)
        ) * -INF
    ...
    def _get_new_finished_state(self, state, new_seq, new_log_probs):
        ...
        new_finished_flags = tf.equal(new_seq[:, :, -1], self.eos_id)
        new_scores += (1.0 - tf.cast(new_finished_flags, dtype=tf.float32)) * -INF
...
def _length_normalization(alpha, length):
    ...
    return tf.pow(((5.0 + tf.cast(length, dtype=tf.float32)) / 6.0), alpha)
