...
class Transformer(object):
    ...
    def __call__(self, inputs, targets=None):
        ...
        mlperf_log.transformer_print(
            key=mlperf_log.MODEL_HP_INITIALIZER_GAIN, value=self.params.initializer_gain
        )
        ...
        with tf.compat.v1.name_scope("decode"):
            ...
            with tf.compat.v1.name_scope("shift_targets"):
                ...
                decoder_inputs = tf.pad(
                    tensor=decoder_inputs, paddings=[[0, 0], [1, 0], [0, 0]]
                )[:, :-1, :]
            with tf.compat.v1.name_scope("add_pos_encoding"):
            ...
            with tf.compat.v1.tpu.bfloat16_scope():
                ...
                decoder_self_attention_bias = tf.cast(
                    model_utils.get_decoder_self_attention_bias(length), tf.bfloat16
                )
...
class LayerNormalization(tf.compat.v1.layers.Layer):
    ...
    def build(self, _):
        mlperf_log.transformer_print(
            key=mlperf_log.MODEL_HP_NORM, value={"hidden_size": self.hidden_size}
        )
    ...
    def call(self, x, epsilon=1e-6):
        ...
        variance = tf.reduce_mean(
            input_tensor=tf.square(x - mean), axis=[-1], keepdims=True
        )
...
class EncoderStack(tf.compat.v1.layers.Layer):
    ...
    def __init__(self, params, train):
        ...
        self.layers = []
        mlperf_log.transformer_print(
            key=mlperf_log.MODEL_HP_NUM_HIDDEN_LAYERS, value=params.num_hidden_layers
        )
