        ...
        with tf.io.gfile.GFile(target_compiled_file, mode="w") as target_writer:
            for i in range(len(raw_files["inputs"])):
                ...
                tf.compat.v1.logging.info(
                    "Reading files %s and %s." % (input_file, target_file)
                )
...
def encode_and_save_files(subtokenizer, data_dir, raw_files, tag, total_shards):
    ...
    filepaths = [
        shard_filename(data_dir, tag, n + 1, total_shards) for n in range(total_shards)
    ]
...
def shard_filename(path, tag, shard_num, total_shards):
    ...
    return os.path.join(
        path,
        "%s-%s-%s-%.5d-of-%.5d" % (_PREFIX, _ENCODE_TAG, tag, shard_num, total_shards),
    )
