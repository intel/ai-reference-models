...
flags.DEFINE_string(
    "in_graph", "fp32_graphdef.pb", """TensorFlow 'GraphDef' file for FP32 to load."""
)
flags.DEFINE_bool(
    "input_binary", True, """Whether the input files are in binary format."""
)
flags.DEFINE_string(
    "vocab_file", "vocab.ende.32768", "Path to subtoken vocabulary file."
)
flags.DEFINE_string("file", "newstest2014.en", """File saved to an output file.""")
flags.DEFINE_string(
    "file_out",
    "translate.txt",
    """If --file flag is specified, save translation to this file.""",
)
flags.DEFINE_integer("batch_size", 64, """The validation batch size""")
flags.DEFINE_integer(
    "num_inter", 1, """Number of sentences to exclude from validation file."""
)
flags.DEFINE_integer(
    "num_intra", 40, """Number of sentences to exclude from validation file."""
)
flags.DEFINE_integer(
    "num_batches", 0, """Number of batches of sentences to run inference for"""
)
flags.DEFINE_bool(
    "sort_input_sentences",
    None,
...
def main(unused_args):
    ...
    with tf.Graph().as_default() as graph:
        y = tf.import_graph_def(
            graph_def, return_elements=["model/Transformer/strided_slice_19:0"], name=""
        )
    end = timer()
        ...
        for i, line in enumerate(batches):
            ...
            if i % FLAGS.batch_size == 0:
                ...
                dec_tensor = sess.run(
                    y,
                    feed_dict={
                        "input_tensor:0": pd.DataFrame(batch)
                        .fillna(0)
