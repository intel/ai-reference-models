...
def run_weights_sharing_model(model, dataloader, augment, tid, opt):
    ...
    with torch.no_grad():
        for batch_i, (img, _, _, shapes) in enumerate(dataloader):
            ...
            if batch_i >= opt.warmup_iterations:
...
def test(opt,
    ...
    if performance:
        ...
        if opt.number_iter > len(dataloader) - 1:
            opt.number_iter = len(dataloader) - 1

        if opt.warmup_iterations > opt.number_iter / 2:
            opt.warmup_iterations = int(opt.number_iter / 2)
            warmup_iterations = opt.warmup_iterations
