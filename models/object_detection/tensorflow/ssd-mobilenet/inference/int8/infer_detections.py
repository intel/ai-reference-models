    ...
    def build_data_sess(self):
        ...
        with preprocess_graph.as_default():
            ...
            with tf.compat.v1.gfile.FastGFile(
                os.path.join(os.path.dirname(dir_path), "ssdmobilenet_preprocess.pb"),
                "rb",
            ) as input_file:
        ...
        with tf.compat.v1.Session(graph=self.infer_graph, config=self.config) as sess:
            ...
            for step in range(total_iter):
                ...
                if self.args.data_location:
                    ...
                    input_images = self.pre_sess.run(
                        self.pre_output, {self.pre_input: input_images}
                    )
    ...
    def accuracy_check(self):
        ...
        with tf.compat.v1.Session(graph=self.infer_graph, config=self.config) as sess:
            ...
            while True:
                ...
                image_id = (
                    image_id[0]
                    if type(image_id[0]) == "str"
                    else image_id[0].decode("utf-8")
                )
