...
parser.add_argument('-s', '--single_image',
                    help='Run for single image onle, if false, run for the whole dataset', action='store_true')
parser.add_argument('-x', '--single_socket',
                    help='Run for single socket, if false, run both sockets', action='store_true')
...
parser.add_argument('--num-inter-threads',
                    help='Number of inter threads', type=int, default=None, dest="num_inter_threads")
parser.add_argument('--num-intra-threads',
                    help='Number of intra threads', type=int, default=None, dest="num_intra_threads")
...
with detection_graph.as_default():
    ...
    with tf.io.gfile.GFile(args.graph, 'rb') as fid:
...
def load_image_into_numpy_array(image):
    ...
    if image.mode == 'L':
        np_image = np.array(image.getdata()).reshape((im_height, im_width)).astype(np.uint8)
...
def run_inference_for_single_image(graph):
    ...
    sess_config.intra_op_parallelism_threads = int(args.num_intra_threads)
    sess_config.inter_op_parallelism_threads = int(args.num_inter_threads)
    print("intra_op_parallelism_threads {}".format(sess_config.intra_op_parallelism_threads))
    ...
    with graph.as_default():
        with tf.compat.v1.Session(config=sess_config) as sess:
            ...
            if args.evaluate_tensor is None:
                ...
                our_op = tf.compat.v1.get_default_graph().get_operation_by_name(args.evaluate_tensor)
            ...
            for index, image_path in enumerate(TEST_IMAGE_PATHS):
                ...
                if args.number_of_steps is not None:
                    if args.single_image:
                        ...
                        print('Avg. Duration per Step:' + str(total_duration / (args.number_of_steps - 20)))
                        break
