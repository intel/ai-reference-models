...
def parse_and_preprocess(serialized_example):
  ...
  image_tensor = tf.io.decode_jpeg(encoded_image, channels=3)
...
class model_infer:
  ...
  def __init__(self):
    ...
    self.output_layer = 'output_boxes'
    self.load_graph()
    ...
    self.output_tensors = self.infer_graph.get_tensor_by_name(self.output_layer + ":0")
    self.category_map_reverse = {v : k for k, v in category_map.items()}
    print(self.category_map_reverse)
  ...
  def load_graph(self):
    ...
    with self.infer_graph.as_default():
      ...
      output_graph = optimize_for_inference(graph_def, [self.input_layer],
                              [self.output_layer], dtypes.float32.as_datatype_enum, False)
  ...
  def accuracy_check(self):
    ...
    with tf.compat.v1.Session(graph=self.infer_graph, config=self.config) as sess:
      ...
      while True:
        ...
        pred_bbox = sess.run(self.output_tensors, {self.input_tensor: input_images})
        boxes = pred_bbox[:, :, 0:4] # Bounding box predictions from model
        pred_conf = pred_bbox[:, :, 4:] # Confidence for each bounding box
        ...
        bboxes = [ground_truth['boxes'], scores.eval(), ground_truth['classes'], valid_detections.eval()]
        
        detection['boxes'] = boxes.eval()[0]
        detection['scores'] = scores.eval()[0]
        detection['classes'] = classes.eval()[0]
        evaluator.add_single_detected_image_info(image_id, detection)
        if iter * self.args.batch_size >= 1:
          evaluator.evaluate()
          break
