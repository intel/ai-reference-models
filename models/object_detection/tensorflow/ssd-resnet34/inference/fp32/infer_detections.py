...
class ssd_resnet34_infer:
    def __init__(self):
        ...
        arg_parser.add_argument(
            "-g", "--input-graph", help="Specify the input graph.", dest="input_graph"
        )
        ...
        self.num_batches = (
            ssd_constants.COCO_NUM_VAL_IMAGES // self.args.batch_size
        ...
        if self.args.input_size == 300:
            ...
            self.output_tensors = [
                self.freeze_graph.get_tensor_by_name(x + ":0") for x in output_layers
            ]
        elif self.args.input_size == 1200:
            with self.freeze_graph.as_default():
                self.input_tensor = tf.compat.v1.get_default_graph().get_tensor_by_name(
                    "image:0"
                )
                self.output_tensors = {}
                for key in [
    ...
    def run_benchmark(self):
        ...
        with tf.compat.v1.Session(graph=self.freeze_graph, config=self.config) as sess:
            ...
            print(
                "Time spent per BATCH: {0:10.4f} ms".format(
                    ttime / total_batches * 1000
                )
            )
            print(
                "Total samples/sec: {0:10.4f} samples/s".format(
    ...
    def __get_input(self):
        ...
        return preprocessor.minibatch(
            self.dataset, subset="validation", params=self.params, shift_ratio=0
        )
    ...
    def load_batch(self, niter, image_lines, image_root, batch_size):
        ...
        while image_index < len(image_lines) and image_index < niter * batch_size:
            ...
            image = np.array(
                image.resize(
                    (self.args.input_size, self.args.input_size), Image.BILINEAR
                )
            )
    ...
    def load_batch_tfrecord(self, niter, data_group, batch_size, sess):
        ...
        image_index = (niter - 1) * batch_size
        while (
            image_index < ssd_constants.COCO_NUM_VAL_IMAGES
            and image_index < niter * batch_size
        ):
        ...
        with graph.as_default():
            with tf.compat.v1.Session() as sess:
                ...
                tfrecord_path = os.path.join(
                    self.args.data_location, "validation-00000-of-00001"
                )
                ...
                for ind in range(num_iter):
                    (
                        ...
                        image_name_batch,
                        image_shape_batch,
                    ) = self.load_batch_tfrecord(
                        ind + 1, data_group, self.args.batch_size, sess
                    )
                    count += image_batch.shape[0]
                    print("process: %d images" % count)
                    output_dict = sess.run(
                        self.output_tensors, feed_dict={self.input_tensor: image_batch}
                    )
                    cur_batch_size = image_batch.shape[0]
                    for bind in range(cur_batch_size):
                        num_detections = int(
                            output_dict["detection_bboxes"][bind].shape[0]
                        )
                        for ind_bb in range(num_detections):
                            record = {}
                            height, width = image_shape_batch[bind]
                            ymin = (
                                output_dict["detection_bboxes"][bind][ind_bb][0]
                                * height
                            )
                            xmin = (
                                output_dict["detection_bboxes"][bind][ind_bb][1] * width
                            )
                            ymax = (
                                output_dict["detection_bboxes"][bind][ind_bb][2]
                                * height
                            )
                            xmax = (
                                output_dict["detection_bboxes"][bind][ind_bb][3] * width
                            )
                            score = output_dict["detection_scores"][bind][ind_bb]
                            class_id = int(
                                output_dict["detection_classes"][bind][ind_bb]
                            )
    ...
    def accuracy_check_1200(self):
        ...
        gt_path = os.path.join(
            self.args.data_location, "annotations", "instances_val2017.json"
        )
