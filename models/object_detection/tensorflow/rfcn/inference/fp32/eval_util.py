...
def _run_checkpoint_once(
    ...
    if inter_op > 0 or intra_op > 0:
        config = tf.compat.v1.ConfigProto(
            inter_op_parallelism_threads=inter_op, intra_op_parallelism_threads=intra_op
        )
    ...
    sess = tf.compat.v1.Session(
        master, graph=tf.compat.v1.get_default_graph(), config=config
    )
    ...
    with tf.compat.v1.train.QueueRunner(sess):
        try:
            ...
            for batch in range(int(num_batches)):
                ...
                if (batch % 100) == 0:
                    logging.info(
                        "Iteration %d: %.3f sec", batch, time.time() - start_time
                    )
                if not result_dict:
                    continue
...
def result_dict_for_single_example(
    ...
    num_detections = tf.cast(
        detections[detection_fields.num_detections][0], dtype=tf.int32
    )
    ...
    if scale_to_absolute:
        ...
        output_dict[detection_fields.detection_boxes] = absolute_detection_boxlist.get()
    else:
    ...
    if groundtruth:
        ...
        if scale_to_absolute:
            ...
            output_dict[input_data_fields.groundtruth_boxes] = absolute_gt_boxlist.get()
...
def get_eval_metric_ops_for_evaluators(
    evaluation_metrics, categories, eval_dict, include_metrics_per_category=False
):
