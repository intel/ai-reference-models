...
class RFCNRunner:
    ...
    def parse_args(self):
        ...
        mutex_group.add_argument(
            "-z",
            "--visualize",
            help="Whether to visulize the output image",
            action="store_true",
        )
        parser.add_argument(
            "-v", "--verbose", help="Print some useful info.", action="store_true"
        )
        parser.add_argument(
            "-t",
            "--timeline",
        ...
        parser.add_argument(
            "-p", "--print_accuracy", help="Print accuracy results", action="store_true"
        )
        parser.add_argument(
            "-g",
            "--input_graph",
            help="The input frozen graph pb file",
            dest="input_graph",
            required=True,
            default=None,
        )
        parser.add_argument(
            "-d",
            "--data_location",
            help="The location of the image data to be analyzed.",
            dest="data_location",
            default=None,
            required=True,
        )
    ...
    def validate_args(self):
        ...
        if not (
            ...
            and os.path.exists(
                os.path.join(self.args.data_location, self.TEST_IMG_FILE)
            )
        ):
            raise ValueError(
                "Unable to locate images for evaluation at {}".format(
                    self.args.data_location
                )
            )
        if os.path.isdir(self.research_dir):
            ...
            if not os.path.exists(self.label_map_file):
                raise ValueError(
                    "Unable to locate label map file at {}".format(self.label_map_file)
                )
        else:
            raise ValueError(
                "{} is not a valid path to the TensorFlow models.".format(
                    self.args.tf_models_path
                )
            )

        if not os.path.exists(self.args.input_graph):
            raise ValueError(
                "Unable to find the input graph protobuf file: {}".format(
    ...
    def finish_import(self):
        ...
        self.label_map_util = importlib.import_module(
            "..label_map_util", package="object_detection.utils.label_map_util"
        )
        self.vis_util = importlib.import_module(
            "..visualization_utils",
            package="object_detection.utils.visualization_utils",
        )
    ...
    def visualize(self, output_dict, image_np):
        ...
        if (
            self.args.visualize
            and self.args.evaluate_tensor is None
            and self.category_index
            and output_dict
            and image_np
        ):
        ...
        with self.detection_graph.as_default():
            with tf.compat.v1.Session(config=sess_config) as sess:
                ...
                if not self.args.evaluate_tensor:
                    ...
                    our_op = tf.compat.v1.get_default_graph().get_operation_by_name(
                        self.args.evaluate_tensor
                    )
                    ...
                    if self.args.visualize:
                        ...
                        if index == self.MAX_STEPS - 1:
                            print(
                                ...
                                + str(total_duration / self.MAX_STEPS)
                            )

                    if self.args.number_of_steps and index == (
                        self.args.number_of_steps - 1
                    ):
                        print(
                            "Avg. Duration per Step:"
