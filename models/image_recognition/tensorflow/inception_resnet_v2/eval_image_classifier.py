...
tf.app.flags.DEFINE_integer(
    'eval_log_frequency', 10,
    'Number of eval steps to run between displaying '
    'eval metrics.')
...
class _LoggerHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if (self._step + 1) % FLAGS.eval_log_frequency == 0:
            ...
            print (
                format_str % (datetime.now(), (self._step+1), images_per_sec))

    def end(self, run_context):
        print(
            'self._total_images_per_sec = %.1f' % self._total_images_per_sec)
