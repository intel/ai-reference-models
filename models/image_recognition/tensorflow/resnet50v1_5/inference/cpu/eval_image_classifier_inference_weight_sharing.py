...
class eval_classifier_optimized_graph:
    ...
    def run(self):
        ...
        if self.args.onednn_graph:
            infer_config.graph_options.rewrite_options.constant_folding = (
        ...
        def run_model(data_sess, infer_sess, tid):
            ...
            if self.args.onednn_graph:
                ...
                print(
                    "Instance num %d Avg Time/Iteration %f msec Throughput %f images/sec"
                    % (
                        tid,
                        time_consume * 1000 / total_examples,
                        total_examples / time_consume,
                    )
                )
            else:
                ...
                print(
                    ...
                    % (
                        tid,
                        time_consume * 1000 / (steps - warm_up),
                        (steps - warm_up) / time_consume,
                    )
                )

        if not self.args.accuracy_only:
            if (
                self.args.onednn_graph
                and os.environ.get("_ITEX_ONEDNN_GRAPH_COMPILER_BACKEND") == "1"
            ):
            ...
            for i in range(1, num_instances + 1):
                thread = threading.Thread(
                    target=run_model, args=(data_sess, infer_sess1, i)
                )
