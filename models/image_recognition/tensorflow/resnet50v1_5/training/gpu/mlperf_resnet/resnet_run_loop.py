...
def get_synth_input_fn(height, width, num_channels, num_classes, use_bfloat16):
    ...
    def input_fn(
        is_training, data_dir, batch_size, *args, **kwargs
...
def learning_rate_with_decay(
    ...
    def learning_rate_fn(global_step):
        ...
        warmup_lr = initial_learning_rate * global_step_f32 / warmup_steps
        return tf.cond(
            pred=global_step_f32 < warmup_steps,
    ...
    def poly_rate_fn(global_step):
        ...
        poly_rate = tf.compat.v1.train.polynomial_decay(
            plr, decay_steps, train_steps - w_steps + 1, power=2.0
        )
...
def resnet_model_fn(
    ...
    num_examples_metric = tf_mlperf_log.sum_metric(
        tensor=tf.shape(input=logits)[0], name=_NUM_EXAMPLES_NAME
    )
    ...
    if label_smoothing != 0.0:
        ...
        cross_entropy = tf.compat.v1.losses.softmax_cross_entropy(
            logits=logits, onehot_labels=one_hot_labels, label_smoothing=label_smoothing
        )
    else:
    ...
    mlperf_log.resnet_print(
        key=mlperf_log.MODEL_EXCLUDE_BN_FROM_L2,
        value=not loss_filter_fn("batch_normalization"),
    )

    # Add weight decay to the loss.
    mlperf_log.resnet_print(key=mlperf_log.MODEL_L2_REGULARIZATION, value=weight_decay)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        learning_rate = tf_mlperf_log.log_deferred(
            op=learning_rate, log_id=log_id, every_n=100
        )
        ...
        mlperf_log.resnet_print(
            key=mlperf_log.OPT_NAME, value=mlperf_log.SGD_WITH_MOMENTUM
        )
        ...
        if loss_scale != 1:
            ...
            scaled_grad_vars = optimizer.compute_gradients(
                loss * loss_scale, gate_gradients=tf.compat.v1.train.Optimizer.GATE_NONE
            )
            ...
            minimize_op = optimizer.minimize(
                loss, global_step, gate_gradients=tf.compat.v1.train.Optimizer.GATE_NONE
            )
...
def resnet_main(seed, flags, model_function, input_function, shape=None):
    ...
    mlperf_log.resnet_print(key=mlperf_log.INPUT_BATCH_SIZE, value=flags.batch_size)
    ...
    if is_mpi:
        ...
        steps_per_eval_per_worker = (
            steps_per_epoch_per_worker * flags.epochs_between_evals
        )
    ...
    for i in range(flags.train_epochs // flags.epochs_between_evals):
        ...
        for j in range(flags.epochs_between_evals):
            mlperf_log.resnet_print(
                key=mlperf_log.TRAIN_EPOCH, value=i * flags.epochs_between_evals + j
            )
        ...
        if is_mpi:
            ...
            if (flags.max_train_steps) and (
                flags.max_train_steps < steps_per_eval_per_worker
            ):
        ...
        mlperf_log.resnet_print(
            key=mlperf_log.EVAL_ACCURACY, value=float(eval_results["accuracy"])
        )
...
class ResnetArgParser(argparse.ArgumentParser):
    ...
    def parse_args(self, args=None, namespace=None):
        args = super(ResnetArgParser, self).parse_args(args=args, namespace=namespace)
