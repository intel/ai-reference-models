...
def input_fn(
    is_training, data_dir, batch_size, num_epochs=1, num_gpus=None, dtype=tf.float32
):
...
def imagenet_model_fn(features, labels, mode, params):
    ...
    return resnet_run_loop.resnet_model_fn(
        ...
        enable_lars=params["enable_lars"],
        use_float16=params["use_float16"],
...
def main(argv):
    ...
    mlperf_log.resnet_print(
        key=mlperf_log.PREPROC_NUM_TRAIN_EXAMPLES, value=_NUM_IMAGES["train"]
    )
    mlperf_log.resnet_print(
        key=mlperf_log.PREPROC_NUM_EVAL_EXAMPLES, value=_NUM_IMAGES["validation"]
    )
