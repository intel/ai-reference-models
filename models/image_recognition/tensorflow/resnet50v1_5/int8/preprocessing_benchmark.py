...
def eval_image(image, height, width, resize_method, central_fraction=0.875, scope=None):
    with tf.compat.v1.name_scope("eval_image"):
        if resize_method == "crop":
            shape = tf.shape(input=image)
            image = tf.cond(
                pred=tf.less(shape[0], shape[1]),
                true_fn=lambda: tf.image.resize(
                    image,
                    tf.convert_to_tensor(
                        value=[256, 256 * shape[1] / shape[0]], dtype=tf.int32
                    ),
                ),
                false_fn=lambda: tf.image.resize(
                    image,
                    tf.convert_to_tensor(
                        value=[256 * shape[0] / shape[1], 256], dtype=tf.int32
                    ),
                ),
            )
            shape = tf.shape(input=image)
            ...
            means = tf.broadcast_to(
                [123.68, 116.78, 103.94], tf.shape(input=distorted_image)
            )
...
class RecordInputImagePreprocessor(object):
    ...
    def __init__(self, height, width, batch_size, num_cores, resize_method):
