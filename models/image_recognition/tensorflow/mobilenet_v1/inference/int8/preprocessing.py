...
def get_image_resize_method(resize_method, batch_position=0):
    ...
    def resize_method_0():
        return utils.smart_cond(
            batch_position % len(resize_methods) == 0,
            lambda: lookup(0),
            resize_method_1,
        )
    ...
    def resize_method_1():
        return utils.smart_cond(
            batch_position % len(resize_methods) == 1,
            lambda: lookup(1),
            resize_method_2,
        )
    ...
    def resize_method_2():
        return utils.smart_cond(
            batch_position % len(resize_methods) == 2,
            lambda: lookup(2),
            lambda: lookup(3),
        )
    ...
    def image_preprocess(self, image_buffer, bbox, batch_position):
        ...
        if self.train:
            ...
            image = preprocess_for_train(
                image_buffer, self.height, self.width, bbox, batch_position
            )
        else:
            ...
            new_height = int(100.0 * self.height / 87.5)
            new_width = int(100.0 * self.width / 87.5)
        ...
        with tf.compat.v1.name_scope("batch_processing"):
            ...
            for split_index in xrange(self.num_splits):
                ...
                labels[split_index] = tf.reshape(
                    labels[split_index], [self.batch_size_per_split]
                )
