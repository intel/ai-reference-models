...
def create_dataset(data_dir, data_name):
    ...
    if data_name not in supported_datasets:
        raise ValueError(
            ...
            ", ".join([key for key in sorted(supported_datasets.keys())]),
        )
...
class Cifar10Data(Dataset):
    ...
    def read_data_files(self, subset="train"):
        ...
        assert self.data_dir, (
            "Cannot call `read_data_files` when using synthetic " "data"
        )
        if subset == "train":
            filenames = [
                os.path.join(self.data_dir, "data_batch_%d" % i) for i in xrange(1, 6)
            ]
        elif subset == "validation":
        ...
        all_images = np.concatenate(
            [each_input["data"] for each_input in inputs]
        ).astype(np.float32)
