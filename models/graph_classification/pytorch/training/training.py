...
def train_homo(model, loader, optimizer, device, amp, progress_bar=True, desc="",
               trim=False):
    if progress_bar:
...
def train_hetero(model, loader, optimizer, device, amp, progress_bar=True, desc="",
                 trim=False):
    if trim:
...
def run(args: argparse.ArgumentParser):
    ...
    for dataset_name in args.datasets:
        ...
        if torch.cuda.is_available():
            ...
            if args.bf16:
                amp = torch.cpu.amp.autocast(enabled=True, dtype=torch.bfloat16)
            elif args.fp16:
                amp = torch.cpu.amp.autocast(enabled=True, dtype=torch.half)
            else:
                amp = torch.cpu.amp.autocast(enabled=False)
        else:
            if args.fp16:
                print(f'PyTorch does not support fp16 amp, will fallback to fp32.')
                ...
                for layers in args.num_layers:
                    ...
                    for hidden_channels in args.num_hidden_channels:
                        ...
                        if args.fp16:
                            dtype = torch.half
                        if args.ipex:
                            model, optimizer = ipex.optimize(model, optimizer=optimizer, dtype=dtype, level='O0', weights_prepack=True)
