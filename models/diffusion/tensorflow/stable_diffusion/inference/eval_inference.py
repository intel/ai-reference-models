...
class eval_stable_diffusion:
    ...
    def run(self):
        ...
        print(
            ...
            + " precision with a batch size of "
        ...
        if not self.args.accuracy_only:
            ...
            print(
                "Avg Throughput: "
                + str(self.args.batch_size / total_time)
                + " examples/sec"
            )
            ...
            if not self.args.data_location:
                exit(
                    "Please provide a path to the COCO dataset to compute accuracy using --data-location arg."
                )

            with open(
                f"{self.args.data_location}/annotations/captions_val2017.json", "r"
            ) as f:
                data = json.load(f)
            ...
            for index, row in captions.iterrows():
                ...
                np_img = np_img.reshape(
                    1, np_img.shape[0], np_img.shape[1], np_img.shape[2]
                )
                scaled_img = self.scale_images(np_img, (512, 512, 3))
                original_imgs.append(scaled_img)
                save_img = scaled_img.reshape(
                    scaled_img.shape[1], scaled_img.shape[2], scaled_img.shape[3]
                )
                im = Image.fromarray(save_img)
                im.save(
                    self.args.output_dir + "/original/" + row["image"].split("/")[-1]
                )

                print(
                    "Generating image with Stable Diffusion for the prompt: ",
                    row["caption"],
                )
                generated_img = model.text_to_image(
                    row["caption"], seed=2023, num_steps=self.args.steps
                )
                generated_imgs.append(generated_img)
                save_img = generated_img.reshape(
                    generated_img.shape[1],
                    generated_img.shape[2],
                    generated_img.shape[3],
                )
                im = Image.fromarray(save_img)
                im.save(
                    self.args.output_dir + "/generated/" + row["image"].split("/")[-1]
                )
                print(
                    "Saved the generated image at: ",
            ...
            original_imgs_np = original_imgs_np.reshape(
                original_imgs_np.shape[0],
                original_imgs_np.shape[2],
                original_imgs_np.shape[3],
                original_imgs_np.shape[4],
            )

            generated_imgs_np = np.array(generated_imgs)
            generated_imgs_np = generated_imgs_np.reshape(
                generated_imgs_np.shape[0],
                generated_imgs_np.shape[2],
                generated_imgs_np.shape[3],
                generated_imgs_np.shape[4],
            )
            ...
            inception_model = InceptionV3(
                include_top=False, pooling="avg", input_shape=(512, 512, 3)
            )
            fid = self.calculate_fid(
                inception_model, original_imgs_np, generated_imgs_np
            )
