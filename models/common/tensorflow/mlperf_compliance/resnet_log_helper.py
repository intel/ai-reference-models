...
def _get_shape(input_tensor):
    return "({})".format(", ".join([str(i) for i in input_tensor.shape.as_list()[1:]]))
...
def log_max_pool(input_tensor, output_tensor):
    mlperf_log.resnet_print(
        ...
        value=_in_out_shape(input_tensor=input_tensor, output_tensor=output_tensor),
...
def log_conv2d(input_tensor, output_tensor, stride, filters, initializer, use_bias):
    mlperf_log.resnet_print(
...
def log_batch_norm(
    input_tensor, output_tensor, momentum, epsilon, center, scale, training
):
