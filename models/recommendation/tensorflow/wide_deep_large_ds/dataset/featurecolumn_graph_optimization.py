...
with graph.as_default():
    ...
    def optimize_crossed_column(nodename, gatherfound):
        ...
        if (
            gatherfound[0] == 1
            and node.op == "Identity"
            and "SparseCross" in node.input[0]
        ):
            return node.name
        elif (
            gatherfound[0] == 1
            and node.op == "Identity"
            and ("hash_table_Lookup" in node.input[0] or "lookup" in node.input[0])
        ):
            return node.name
        elif (
            gatherfound[0] == 2
            and node.op == "SparseFillEmptyRows"
            and "GatherV2" in node.input[0]
        ):
            return node.name
        elif (
            gatherfound[0] == 2
            and node.op == "GatherV2"
            and "Unique" in node.input[1]
            and "Identity" not in node.input[0]
        ):
    ...
    def optimize_categorical_with_voc_list(nodename, gatherfound):
        ...
        if (
            gatherfound[0] == 1
            and node.op == "Select"
            and "Add" in node.input[2]
            and "hash_table_Lookup" in node.input[1]
        ):
            return node.name
        if node.op == "GatherV2" and "Unique" in node.input[1]:
            gatherfound[0] = 1
    ...
    def optimize_numeric(nodename):
        if ":" in nodename:
            nodename = nodename.split(":")[0]
        node = name_node_dict[nodename]
        if node.op == "Reshape" and (
            "ParseExample" in node.input[0] or "div" in node.input[0]
        ):
            return node.input[0]
    ...
    def fuse_categorical_numeric_columns():
        new_categorical_placeholder = tf.compat.v1.placeholder(
            tf.int64, shape=(None, None), name="new_categorical_placeholder"
        )
        new_numeric_placeholder = tf.compat.v1.placeholder(
            tf.float32, shape=(None, None), name="new_numeric_placeholder"
        )
        ...
        full_weights_constant_categorical = tf.constant(
            fused_categorical_weights_const, name="full_weights_constant_categorical"
        )
        batch_gather_op = tf.gather_nd(
            full_weights_constant_categorical,
        ...
        full_weights_constant_embedding = tf.constant(
            fused_embedding_weights_const, name="full_weights_constant_embedding"
        )
        batch_gather_op_embedding = tf.gather_nd(
            full_weights_constant_embedding,
            new_categorical_placeholder,
            name="gather_embedding_weights",
        )
        embedding_reshape = tf.reshape(
            batch_gather_op_embedding, shape=[-1, 32 * 26], name="embedding_reshape"
        )
        real_div_input_tens_list = [embedding_reshape, new_numeric_placeholder]
        new_concat_node = tf.concat(
            real_div_input_tens_list, name="new_concat_node", axis=1
        )
    ...
    for node in old_graph_def.node:
        ...
        if (
            node.op == "ConcatV2"
            and "dnn/input_from_feature_columns" in nodename
            and "input_layer/concat" in nodename
        ):
    ...
    try:
        for i, inputname in enumerate(weightsumnobias_node.input):
            if "weighted_by" not in inputname and "_X_" not in inputname:
                gatherfound[0] = 0
                res = optimize_categorical_with_voc_list(
                ...
                if res:
                    ...
                    if res:
                        ...
                        if res:
                            ...
                            gatherfound[0] = 0
                            res = optimize_bucketized_column(
                                weightsumnobias_node.input[i], gatherfound
                            )
                            if res:
                                weightsumnobias_node.input[i] = res
        ...
        for i, inputname in enumerate(dnn_concat_node.input):
            if (
                "_embedding" in inputname
                and "shared_embedding" not in inputname
                and "weighted_by" not in inputname
                and "_X_" not in inputname
            ):
                gatherfound[0] = 0
                res = optimize_categorical_with_voc_list(
        ...
        print(
            "Cannot optimize the given graph. The given graph might be an optimized one"
        )
        print(
            "--------------------------------------------------------------------------"
        )
...
new_graph_def = tf.compat.v1.graph_util.extract_sub_graph(old_graph_def, output_nodes)

filename = args.output_graph
graph_io.write_graph(
    new_graph_def, os.path.dirname(filename), os.path.basename(filename), as_text=False
)
