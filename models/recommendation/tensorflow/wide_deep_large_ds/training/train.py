...
def build_feature_cols(train_file_path, test_file_path):
    ...
    def numeric_column_normalized(column_name, normalizer_fn):
        return tf.feature_column.numeric_column(
            column_name, normalizer_fn=normalizer_fn
        )
    ...
    for i in range(len(CONTINUOUS_COLUMNS)):
        ...
        deep_columns.append(
            numeric_column_normalized(CONTINUOUS_COLUMNS[i], normalizer_fn)
        )
    wide_columns = []
    ...
    for col in wide_columns:
        deep_columns.append(tf.feature_column.embedding_column(col, dimension=32))
...
def build_model(model_type, model_dir, wide_columns, deep_columns):
    ...
    if model_type == "WIDE":
        m = tf.estimator.LinearClassifier(
            config=runconfig, model_dir=model_dir, feature_columns=wide_columns
        )
...
def train_and_eval():
    ...
    m.train(
        input_fn=lambda: generate_input_fn(train_file, batch_size, int(no_of_epochs)),
        steps=int(train_steps),
    )
    print("fit done")
    results = m.evaluate(
        input_fn=lambda: generate_input_fn(test_file, batch_size, 1), steps=test_steps
    )
    print("evaluate done")

    export_folder = m.export_saved_model(export_dir, serving_input_fn)
...
def get_arg_parser():
    ...
    parser.add_argument(
        "--data_location", help="Full path of train data", required=True
    )
    parser.add_argument(
        "--steps",
