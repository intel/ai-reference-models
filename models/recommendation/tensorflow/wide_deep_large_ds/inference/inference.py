...
placeholder_list = [
    "import/new_numeric_placeholder:0",
...
with tf.compat.v1.Session(config=config, graph=graph) as sess1:
    ...
    while True:
        ...
        logistic = sess1.run(
            output_tensor, dict(zip(input_tensor, features_list[i][0:2]))
        )
        if i > warm_iter:
        ...
        if args.compute_accuracy:
            ...
            correctly_predicted = correctly_predicted + np.sum(
                features_list[i][2] == predicted_labels
            )
...
latency = (
    ...
    * float(evaluate_duration)
    / float(no_of_test_samples - warm_iter * batch_size)
)
