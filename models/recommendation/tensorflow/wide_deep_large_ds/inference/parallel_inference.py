...
def input_fn(data_file, num_epochs, shuffle, batch_size):
    ...
    def _parse_function(proto):
        ...
        parsed_feature_vals_num = [
            tf.reshape(parsed_features["numeric_1"], shape=[-1, 13])
        ]
        parsed_feature_vals_str = [
            tf.reshape(parsed_features["string_1"], shape=[-1, 2])
            for i in string_feature_names
        ]
        parsed_feature_vals = parsed_feature_vals_num + parsed_feature_vals_str
        if args.compute_accuracy:
            parsed_feature_vals_label = [
                tf.reshape(parsed_features[i], shape=[-1]) for i in ["label"]
            ]
...
no_of_test_samples = sum(
    1 for _ in tf.compat.v1.python_io.tf_record_iterator(data_file)
)
...
with graph.as_default():
    ...
    for node in old_graph_def.node:
        ...
        if k == "IteratorGetNext":
            iterator_node = node
        elif (node.op == "GatherNd" or node.op == "ConcatV2") and (
            placeholder_name in node.input[1]
            or categorical_placeholder in node.input[1]
        ):
            if (
                node.op == "GatherNd"
                and node.name == "import/gather_categorical_weights"
            ):
                gather_categorical_node = node
            elif (
                node.op == "GatherNd" and node.name == "import/gather_embedding_weights"
            ):
...
new_graph_def = tf.compat.v1.graph_util.extract_sub_graph(
    old_graph_def, [output_probabilities_node]
)
...
with graph.as_default():
    ...
    def _body(i, arr):
        ...
        if args.compute_accuracy:
            ...
            num_correct_predictions_batch = tf.reduce_sum(
                tf.cast(correctly_predicted_bool, tf.int32)
            )
        else:
    ...
    i, arr = tf.compat.v2.while_loop(
        cond=lambda i, x: i < int(no_of_batches),
        body=_body,
        loop_vars=[i, arr],
        parallel_iterations=num_parallel_batches,
    )
    array_gather = arr.gather(
        tf.range(0, int(no_of_batches), delta=1, dtype=None, name="range")
    )
...
latency = (
    1000
    * float(batch_size * num_parallel_batches)
    * float(evaluate_duration)
    / float(no_of_test_samples)
)
