...
from training import ncf_common
...
def construct_model(user_input, item_input, params):
  ...
  if params["use_tpu"]:
    ...
    # GMF part
    mf_user_latent = tf.keras.layers.Lambda(
        mf_slice_fn, name="embedding_user_mf")(mf_embedding_user)
    mf_item_latent = tf.keras.layers.Lambda(
        mf_slice_fn, name="embedding_item_mf")(mf_embedding_item)

    # MLP part
    mlp_user_latent = tf.keras.layers.Lambda(
        mlp_slice_fn, name="embedding_user_mlp")(mlp_embedding_user)
    mlp_item_latent = tf.keras.layers.Lambda(
        mlp_slice_fn, name="embedding_item_mlp")(mlp_embedding_item)

  policy = None
  if params["use_bfloat16"]:
    policy = tf.keras.mixed_precision.experimental.Policy('mixed_bfloat16')
    mf_user_latent = tf.cast(mf_user_latent, tf.bfloat16)
    mf_item_latent = tf.cast(mf_item_latent, tf.bfloat16)
    mlp_user_latent = tf.cast(mlp_user_latent, tf.bfloat16)
    mlp_item_latent = tf.cast(mlp_item_latent, tf.bfloat16)
