...
def _get_in_memory_dataloader(
    ...
    # criteo_1tb code path uses below two conditionals
        ...
        stage_files: List[List[str]] = [
            ...
            # for crop dataset
            [os.path.join(dir_path, f"day_{i}_dense.npy") for i in range(DAYS)],
            [os.path.join(dir_path, f"day_{i}_{sparse_part}") for i in range(DAYS)],
            [os.path.join(dir_path, f"day_{i}_labels.npy") for i in range(DAYS)],
        ]
