...
def visualize_embeddings_umap(emb_l, output_dir="", max_size=500000):
...
def create_vis_data(dlrm, data_ld, max_size=50000, info=""):
    ...
    for j, (X, lS_o, lS_i, T) in enumerate(data_ld):
        ...
        if int(z.detach().cpu().numpy()[0, 0] + 0.5) == int(
            T.detach().cpu().numpy()[0, 0]
        ):
            all_c.append(0)
        else:
            all_c.append(1)
...
def plot_all_data(
    ...
    ax1.title.set_text(
        "Train (" + str(len(train_labels)) + " of " + total_train_size + ")"
    )
    if test_data is not None and test_labels is not None:
        ...
        ax2.title.set_text(
            "Test (" + str(len(test_labels)) + " of " + total_test_size + ")"
        )
...
def plot_one_class(
    ...
    ax1.scatter(
        -Y_train_l[:, 0], -Y_train_l[:, 1], s=size, c=col, marker=".", linewidth=0
    )
    ax1.title.set_text(
        "Train, (" + str(len(train_labels)) + " of " + total_train_size + ")"
    )
...
def visualize_data_umap(
    dlrm, train_data_ld, test_data_ld=None, max_umap_size=50000, output_dir=""
):

    train_feat, train_X, train_cat, train_T, train_z, train_c = create_vis_data(
        dlrm=dlrm, data_ld=train_data_ld, max_size=max_umap_size, info="train"
    )
    ...
    if test_data_ld is not None:
        test_feat, test_X, test_cat, test_T, test_z, test_c = create_vis_data(
            dlrm=dlrm, data_ld=test_data_ld, max_size=max_umap_size, info="test"
        )
...
def analyze_model_data(
    ...
    if skip_embedding == False:
        visualize_embeddings_umap(
            emb_l=dlrm.emb_l, output_dir=output_dir, max_size=max_umap_size
        )
    ...
    if skip_data_plots == False:
        visualize_data_umap(
            dlrm=dlrm,
            train_data_ld=train_ld,
            test_data_ld=test_ld,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-randomize", type=str, default="none"
    )  # total or day or none
