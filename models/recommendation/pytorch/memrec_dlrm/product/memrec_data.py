...
def initialize_globals(D_value, dw_value, K_value, kw_value, codebook_size, bs):
    global random_codebook, D, dw, K, kw, lS_o_preconstructed
    D = D_value
    dw = dw_value
    K = K_value
    kw = kw_value
    num_ones_te = K_value*26
    num_ones_we = kw_value*26
    lS_o_K = [i*num_ones_te for i in range(bs)]
    lS_o_kw = [i*num_ones_we for i in range(bs)]
    lS_o_preconstructed = torch.tensor(lS_o_K + lS_o_kw, dtype=torch.int32)
...
# memrec - collate_wrapper_criteo_offset_tb
def collate_wrapper_criteo_offset_tb(list_of_tuples, args):
    # where each tuple is (X_int, X_cat, y)
    X_int = list_of_tuples[0]
    #lS_o = torch.transpose(list_of_tuples[1], 0, 1)
    lS_i = torch.transpose(list_of_tuples[2], 0, 1)
    T = list_of_tuples[3]
    batchSize = lS_i.shape[0]
    featureCnt = lS_i.shape[1]
    num_ones_te=(K*featureCnt)
    num_ones_we=(kw*featureCnt)

    hd_encoded_cat_data_ = memrec_encoder.wyh_hash_array_sparse(lS_i.detach().cpu().numpy(), args.D, 8, featureCnt, args.K, args.kw, args.hash_tech_id)
    lS_i = torch.tensor(hd_encoded_cat_data_, dtype=torch.int32)
    
    if (args.inference_only):
        #lS_o =  lS_o_preconstructed
        lS_o_K = [i*num_ones_te for i in range(batchSize)]
        lS_o_kw = [i*num_ones_we for i in range(batchSize)]  
        lS_o = torch.tensor(lS_o_K + lS_o_kw, dtype=torch.int32)
    else:
        lS_o_K = [i*num_ones_te for i in range(batchSize)]
        lS_o_kw = [i*num_ones_we for i in range(batchSize)]  
        lS_o = torch.tensor(lS_o_K + lS_o_kw, dtype=torch.int32)
...
# memrec - collate_wrapper_criteo_offset
def collate_wrapper_criteo_offset(list_of_tuples, args):
    # where each tuple is (X_int, X_cat, y)
    ...
    hd_encoded_cat_data_ = memrec_encoder.wyh_hash_array_sparse(X_cat.detach().cpu().numpy(), args.D, 8, featureCnt, args.K, args.kw, args.hash_tech_id)
    lS_i = torch.tensor(hd_encoded_cat_data_, dtype=torch.int32)
...
def make_criteo_data_and_loaders(args, offset_to_length_converter=False):
    if args.memory_map and args.data_set == "terabyte":
        ...
        collate_wrapper_criteo = collate_wrapper_criteo_offset_tb
        ...
        if args.mlperf_bin_loader:
            ...
            train_loader = torch.utils.data.DataLoader(
                ...
                shuffle=False,
                num_workers=0,
                collate_fn=lambda b: collate_wrapper_criteo(b, args),
                pin_memory=False,
                drop_last=False,
