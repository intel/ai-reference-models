...
class DLRM_Net(nn.Module):
    ...
    def create_emb_bloomrec(self):
        n = D
        m = np.fromstring(args.arch_mlp_bot_sparse, dtype=int, sep="-")[0]
        ...
        n_dash = dw
        m_dash = 1
        EE_dash = nn.EmbeddingBag(n_dash, m_dash, mode="sum", sparse=True)
        W_dash = np.random.uniform(low=-np.sqrt(1 / n), high=np.sqrt(1 / n), size=(n_dash, m_dash)).astype(np.float32)
        EE_dash.weight.data = torch.tensor(W_dash, requires_grad=True)
    ...
    def __init__(
        ...
        if ext_dist.my_size > 1:
            ...
            self.emb_l, self.emb_w = self.create_emb_bloomrec()
    ...
    def apply_emb_bloomrec(self, lS_o, lS_i, emb_l, emb_w):
        #lS_i : batch_size*26*(K+kw) : First K fraction entries is for 1st etable, remaining for the 2nd etable
        #lS_0 : two equal halves of batch size, exact values of each halves depends on K and kw
        first_part_i = int(int(lS_i.size()[0])*self.K/(self.K+self.kw))
        first_part_o = int(int(lS_o.size()[0])/2)

        E = emb_l
        lS_i_ = lS_i[0:first_part_i]
        lS_o_ = lS_o[0:first_part_o]
        V = E(lS_i_.type(torch.LongTensor), lS_o_.type(torch.LongTensor))

        E_w = emb_w
        lS_i_w = lS_i[first_part_i:]
        lS_o_w = lS_o[first_part_o:]
        V_w = E_w(lS_i_w.type(torch.LongTensor), lS_o_w.type(torch.LongTensor))
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        ...
        ly = self.apply_emb_bloomrec(lS_o, lS_i, self.emb_l, self.emb_w)
...
def run():
    ...
    parser.add_argument("--dw", type=int, default=10000)
    parser.add_argument("--K", type=int, default=9)
    parser.add_argument("--kw", type=int, default=1)
    parser.add_argument("--hash-tech-id", type=int, default=2) #2 for wyhash
    parser.add_argument("--random-codebook-size", type=int, default=100000)
    parser.add_argument("--arch-mlp-bot-sparse", type=dash_separated_ints, default="512")
    # intel
    ...
    D = args.D
    dw = args.dw
    dp.initialize_globals(args.D, args.dw, args.K, args.kw, args.random_codebook_size, args.test_mini_batch_size)
    ...
    arch_mlp_bot_sparse_adjusted = str(D) + "-" + args.arch_mlp_bot_sparse
    ln_bot_sparse = np.fromstring(arch_mlp_bot_sparse_adjusted, dtype=int, sep="-")
    # input data
    ...
    m_sparse_out = ln_bot_sparse[ln_bot_sparse.size - 1]
    # approach 1: all
    # num_int = num_fea * num_fea + m_den_out
    # approach 2: unique


    num_int = m_sparse_out + m_den_out
    ...
    if not (args.load_model == ""):
        ...
        if not args.inference_only:
            ...
            if (args.num_batches >= nbatches_test) or (args.num_batches == 0):
                nbatches = nbatches_test-1
            args.test_freq = 0
