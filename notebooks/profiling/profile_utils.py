...
class AIReferenceConfigFile:
    ...
    def __init__(self, confpath='profiling/ai_reference_models.ini'):
        ...
        for each_section in config.sections():
            if each_section == model_name:
                for each_key, each_val in config.items(each_section):
                    ...
                    elif each_key == 'model-name':
                        if each_val is not None:
                            self.model_name = each_val
                    elif each_key == 'ai-type':
                        if each_val is not None:
                            self.ai_type = each_val
                    elif each_key == 'script':
                        if each_val is not None:
                            self.script = each_val
                    elif each_key == 'mode':
                        if each_val is not None:
                            self.mode = each_val
                    elif each_key == 'framework':
                        if each_val is not None:
                            self.framework = each_val
                    elif each_key == 'device':
                        if each_val is not None:
                            self.device = each_val
                    elif each_key == 'precision':
                        if each_val is not None:
                            self.precision = each_val
