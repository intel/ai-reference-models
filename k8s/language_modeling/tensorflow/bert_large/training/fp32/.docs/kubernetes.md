...
```
bert-large-fp32-training/training/fp32/mlops/single-node/kustomization.yaml
bert-large-fp32-training/training/fp32/mlops/multi-node/kustomization.yaml
```
...
The k8 resources needed to run the multi-node bert-large-fp32-training quickstart require deployment of the mpi-operator.
...
of running bert-large-fp32-training multi-node (distributed training) or single-node. 
